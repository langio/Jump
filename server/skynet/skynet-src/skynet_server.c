...
//返回name所标明的模块，模块存在就返回已有的，不存在就从磁盘加载，并进行相关的初始化工作然后返回
struct skynet_context *
...
//将消息入队到handle对应context的消息队列的尾部，若消息队列满了，则进行扩容，若消息队列不在全局队列中，则把消息队列入队到全局队列的尾部
...
//设置handle对应context的endless为true
...
//检查是否是远程消息，harbor不为NULL的话，该handle的harbor存储在*harbor中
		...
		struct skynet_message *msg)
{
	...
	if (ctx->logfile)
	{
		skynet_log_output(ctx->logfile, msg->source, type, msg->session, msg->data,
				sz);
	}
...
//从全局队列Q头部取一个消息队列，然后处理该消息队列中的第一个消息，然后把该消息队列入队到Q的尾部，然后从Q头部取1个消息队列返回
...
//根据name查找handle，如果name的第一个字符是“.”，那么将name当做名字来查找handle，
//如果name的第一个字符是“:”，那么将name转为十六进制数当做handle
...
//将context的handle加入到定时器，param是当前时间
static const char *
...
//将param解析为name（以.为前缀）和handle_id，然后插入到storage中
static const char *
...
//将param转为handle。param[0]为“:”直接将param转为16进制数作为handle；param[0]为“.”，将param作为handle别名查找handle
...
//通过param找到handle，然后将handle表示的服务退出
static const char *
...
//如果context->endless为true，那么将endless设为false，并将result设为“1”并返回该指针
static const char *
...
//将param解析为信号，调用context的信号处理函数来处理
static const char *
...
//调用cmd对应的命令
const char *
skynet_command(struct skynet_context * context, const char * cmd,
		const char * param)
{
...
//根据type对session和data做相应处理（1.分配session   2.为data分配存储空间）
static void _filter_args(struct skynet_context * context, int type,
		...
		uint32_t destination, int type, int session, void * data, size_t sz)
{
	...
	{
		skynet_error(context, "The message to %x is too large (sz = %lu)", destination,
				sz);
		skynet_free(data);
		return -1;
	}
...
int skynet_sendname(struct skynet_context * context, uint32_t source,
		const char * addr, int type, int session, void * data, size_t sz)
{
	if (source == 0)
	{
		source = context->handle;
...
//将消息msg发送给ctx（消息msg入队到ctx的消息队列中）
