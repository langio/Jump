...
static int reserve_id(struct socket_server *ss)
{
	...
	for (i = 0; i < MAX_SOCKET; i++)
	{
		...
		{
			if (__sync_bool_compare_and_swap(&s->type, SOCKET_TYPE_INVALID,
					SOCKET_TYPE_RESERVE))
			{
				s->id = id;
				s->fd = -1;
		...
		struct socket_message *result)
{
	...
	{
		sock = socket(ai_ptr->ai_family, ai_ptr->ai_socktype,
				ai_ptr->ai_protocol);
		if (sock < 0)
		{
			continue;
		...
		const uint8_t udp_address[UDP_ADDRESS_SIZE])
{
	...
	struct write_buffer *buf = append_sendbuffer_(ss, wl, request,
			SIZEOF_UDPBUFFER, 0);
...
{
	struct write_buffer
	*buf = append_sendbuffer_(ss, &s->high, request, SIZEOF_TCPBUFFER, n);
	s->wb_size += buf->sz;
}
...
{
	struct write_buffer
	*buf = append_sendbuffer_(ss, &s->low, request, SIZEOF_TCPBUFFER, 0);
	s->wb_size += buf->sz;
}
		...
		struct socket_message *result, int priority, const uint8_t *udp_address)
{
	...
	if (s->type == SOCKET_TYPE_INVALID
			|| s->id != id || s->type == SOCKET_TYPE_HALFCLOSE
		...
		if (s->protocol == PROTOCOL_TCP)
		{
			...
			if (n < 0)
			{
				switch (errno)
				{
					...
					default:
						fprintf(stderr,
								"socket-server: write to %d (fd=%d) error :%s.\n",
		...
		struct request_listen * request, struct socket_message *result)
{
	...
	struct socket *s = new_fd(ss, id, listen_fd, PROTOCOL_TCP, request->opaque,
			false);
	if (s == NULL)
	{
		goto _failed;
		...
		struct socket_message *result)
{
	...
	result->opaque = request->opaque;
	result->ud = 0;
	struct socket *s = new_fd(ss, id, request->fd, PROTOCOL_TCP,
			request->opaque, true);
	if (s == NULL)
	{
		result->data = NULL;
		...
		struct socket_message *result)
{
	...
	if (s->type == SOCKET_TYPE_PACCEPT || s->type == SOCKET_TYPE_PLISTEN)
	{
		...
		s->type =
				(s->type == SOCKET_TYPE_PACCEPT) ?
						SOCKET_TYPE_CONNECTED : SOCKET_TYPE_LISTEN;
...
static int ctrl_cmd(struct socket_server *ss, struct socket_message *result)
{
	...
	switch (type)
	{
		...
		case 'A':
		{
			...
			return send_socket(ss, &rsu->send, result, PRIORITY_HIGH,
					rsu->address);
		}
		case 'C':
		...
		struct socket_message *result)
{
	...
	struct socket *ns = new_fd(ss, id, client_fd, PROTOCOL_TCP, s->opaque,
			false);
	if (ns == NULL)
	{
		close(client_fd);
	...
	void * sin_addr =
			(u.s.sa_family == AF_INET) ?
					(void*) &u.v4.sin_addr : (void *) &u.v6.sin6_addr;
	int sin_port = ntohs(
			(u.s.sa_family == AF_INET) ? u.v4.sin_port : u.v6.sin6_port);
...
static int do_bind(const char *host, int port, int protocol, int *family)
{
	...
	*family = ai_list->ai_family;
	...
	status = bind(fd, (struct sockaddr *) ai_list->ai_addr,
			ai_list->ai_addrlen);
	if (status != 0)
		goto _failed;
		...
		const char * addr, int port)
{
	...
	int addrsz = gen_udp_address(protocol,
			(union sockaddr_all *) ai_list->ai_addr, request.u.set_udp.address);
