		...
		struct harbor_msg * m)
{
	...
	{
		struct harbor_msg * new_buffer = skynet_malloc(
				queue->size * 2 * sizeof(struct harbor_msg));
		int i;
...
static void push_queue(struct harbor_msg_queue * queue, void * buffer,
		size_t sz, struct remote_message_header * header)
{
	struct harbor_msg m;
	m.header = *header;
	m.buffer = buffer;
...
static inline void header_to_message(
		const struct remote_message_header * header, uint8_t * message)
{
...
static void forward_local_messsage(struct harbor *h, void *msg, int sz)
{
	...
	destination = (destination & HANDLE_MASK)
			| ((uint32_t) h->id << HANDLE_REMOTE_SHIFT);

	if (skynet_send(h->ctx, header.source, destination, type,
			(int) header.session, (void *) msg, sz - HEADER_COOKIE_LENGTH) < 0)
	{
		skynet_send(h->ctx, destination, header.source, PTYPE_ERROR,
				(int) header.session, NULL, 0);
		skynet_error(h->ctx, "Unknown destination :%x from :%x", destination,
				header.source);
	}
}

static void send_remote(struct skynet_context * ctx, int fd,
		const char * buffer, size_t sz, struct remote_message_header * cookie)
{
	...
	for (;;)
	{
		switch (s->status)
		{
			case STATUS_HANDSHAKE:
			{
				...
				{
					skynet_error(h->ctx,
							"Invalid shakehand id (%d) from fd = %d , harbor = %d",
							id, fd, remote_id);
					close_harbor(h, id);
					return;
				}
			...
			case STATUS_HEADER:
			{
				...
				else
				{
					...
					{
						skynet_error(h->ctx,
								"Message is too long from harbor %d", id);
						close_harbor(h, id);
						return;
					}
...
static int remote_send_handle(struct harbor *h, uint32_t source,
		uint32_t destination, int type, int session, const char * msg,
		size_t sz)
{
...
static int remote_send_name(struct harbor *h, uint32_t source,
		const char name[GLOBALNAME_LENGTH], int type, int session,
		...
		int session, uint32_t source, const void * msg, size_t sz)
{
	...
	switch (type)
	{
		case PTYPE_SOCKET:
		{
			...
			switch (message->type)
			{
				...
				case SKYNET_SOCKET_TYPE_CLOSE:
				{
					...
					{
						skynet_error(context, "Unkown fd (%d) closed",
								message->id);
					}
					break;
				}
				case SKYNET_SOCKET_TYPE_CONNECT:
					...
					break;
				default:
					skynet_error(context, "recv invalid socket message type %d",
							type);
					break;
			}
			return 0;
		...
		default:
		{
			...
			{
				if (remote_send_name(h, source, rmsg->destination.name, type,
			...
			else
			{
				if (remote_send_handle(h, source, rmsg->destination.handle,
						type, session, rmsg->message, rmsg->sz))
				{
					return 0;
				}
			}
