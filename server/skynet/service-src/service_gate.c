...
static void _forward(struct gate *g, struct connection * c, int size)
{
	...
	if (g->broker)
	{
		...
		skynet_send(ctx, 0, g->broker, g->client_tag | PTYPE_TAG_DONTCOPY, 0,
				temp, size);
		return;
	}
	if (c->agent)
	{
		...
		skynet_send(ctx, c->client, c->agent,
				g->client_tag | PTYPE_TAG_DONTCOPY, 0, temp, size);
	}
	else if (g->watchdog)
	{
		...
		skynet_send(ctx, 0, g->watchdog, PTYPE_TEXT | PTYPE_TAG_DONTCOPY, 0,
				tmp, size + n);
	}
}
	...
	switch (message->type)
	{
		case SKYNET_SOCKET_TYPE_DATA:
		{
			...
			if (id >= 0)
			{
				...
				dispatch_message(g, c, message->id, message->buffer,
						message->ud);
			}
			else
			{
...
//						c->remote_name);
		...
		case SKYNET_SOCKET_TYPE_ACCEPT:
			...
			{
				struct connection *c = &g->conn[hashid_insert(&g->hash,
						message->ud)];
		...
		uint32_t source, const void * msg, size_t sz)
{
	...
	switch (type)
	{
		...
		case PTYPE_CLIENT:
		{
			...
			uint32_t uid = idbuf[0] | idbuf[1] << 8 | idbuf[2] << 16
					| idbuf[3] << 24;
			int id = hashid_lookup(&g->hash, uid);
			if (id >= 0)
			{
			...
			{
				skynet_error(ctx, "Invalid client id %d from %x", (int) uid,
						source);
				break;
			}
		}
...
int gate_init(struct gate *g, struct skynet_context * ctx, char * parm)
{
	...
	int client_tag = 0;
	char header;
	int n = sscanf(parm, "%c %s %s %d %d %d", &header, watchdog, binding,
			&client_tag, &max, &buffer);
	if (n < 4)
	{
