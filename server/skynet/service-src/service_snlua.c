...
static void _report_launcher_error(struct skynet_context *ctx)
{
	// sizeof "ERROR" == 5
...
static int _init(struct snlua *l, struct skynet_context *ctx, const char * args,
		size_t sz)
{
	lua_State *L = l->L;
	l->ctx = ctx;
	...
	const char *path = optstring(ctx, "lua_path",
...
static int _launch(struct skynet_context * context, void *ud, int type,
