...
static int _cb(struct skynet_context * context, void * ud, int type,
	...
	switch (r)
	{
		case LUA_ERRRUN:
			skynet_error(context,
					"lua call [%x to %s : %d msgsz = %d] error : " KRED "%s" KNRM,
		...
		case LUA_ERRMEM:
			...
			break;
		case LUA_ERRERR:
			skynet_error(context, "lua error in error : [%x to %s : %d]",
					source, self, session);
			break;
		case LUA_ERRGCMM:
			skynet_error(context, "lua gc error : [%x to %s : %d]", source,
					self, session);
			break;
	};
...
{
	struct skynet_context * context = lua_touserdata(L, lua_upvalueindex(1));
	int session = skynet_send(context, 0, 0, PTYPE_TAG_ALLOCSESSION, 0, NULL,
			0);
	lua_pushinteger(L, session);
	return 1;
}
...
static int _send(lua_State *L)
{
	...
	switch (mtype)
	{
		case LUA_TSTRING:
		{
			...
			if (dest_string)
			{
				session = skynet_sendname(context, 0, dest_string, type,
						session, msg, len);
			}
			else
			{
				session = skynet_send(context, 0, dest, type, session, msg,
						len);
			}
			break;
		}
		case LUA_TLIGHTUSERDATA:
		{
			...
			if (dest_string)
			{
				session = skynet_sendname(context, 0, dest_string,
...
static int _redirect(lua_State *L)
{
	...
	switch (mtype)
	{
		case LUA_TSTRING:
		{
			...
			{
				session = skynet_send(context, source, dest, type, session, msg,
						len);
			}
			break;
		}
		...
		default:
			luaL_error(L, "skynet.redirect invalid param %s",
