...
#include "util/xc_socket.h"
#include "util/xc_common.h"

namespace xutil
{

XC_Socket::XC_Socket() : _sock(INVALID_SOCKET), _bOwner(true), _iDomain(AF_INET)
{
}
...
void XC_Socket::init(int fd, bool bOwner, int iDomain)
{
    if(_bOwner)
    {
        close();
...
void XC_Socket::createSocket(int iSocketType, int iDomain)
{
    ...
    if(_sock < 0)
    {
        ...
        throw XC_Socket_Exception("[XC_Socket::createSocket] create socket error! :" + string(strerror(errno)));
    }
}

void XC_Socket::getPeerName(string &sPeerAddress, uint16_t &iPeerPort)
{
...
void XC_Socket::getPeerName(string &sPathName)
{
...
void XC_Socket::getPeerName(struct sockaddr *pstPeerAddr, socklen_t &iPeerLen)
{
    if(getpeername(_sock, pstPeerAddr, &iPeerLen) < 0)
    {
        throw XC_Socket_Exception("[XC_Socket::getPeerName] getpeername error", errno);
    }
}

void XC_Socket::getSockName(string &sSockAddress, uint16_t &iSockPort)
{
    assert(_iDomain == AF_INET);
...
void XC_Socket::getSockName(string &sPathName)
{
    assert(_iDomain == AF_LOCAL);
...
void XC_Socket::getSockName(struct sockaddr *pstSockAddr, socklen_t &iSockLen)
{
    if(getsockname(_sock, pstSockAddr, &iSockLen) < 0)
    {
        throw XC_Socket_Exception("[XC_Socket::getSockName] getsockname error", errno);
    }
}

int XC_Socket::accept(XC_Socket &tcSock, struct sockaddr *pstSockAddr, socklen_t &iSockLen)
{
...
void XC_Socket::parseAddr(const string &sAddr, struct in_addr &stSinAddr)
{
    int iRet = inet_pton(AF_INET, sAddr.c_str(), &stSinAddr);
    if(iRet < 0)
    {
        throw XC_Socket_Exception("[XC_Socket::parseAddr] inet_pton error", errno);
    }
    else if(iRet == 0)
    {
        ...
        if (pstHostent == NULL)
        {
            throw XC_Socket_Exception("[XC_Socket::parseAddr] gethostbyname_r error! :" + string(hstrerror(iError)));
        }
        else
        {
...
void XC_Socket::bind(const string &sServerAddr, int port)
{
    assert(_iDomain == AF_INET);
    ...
    {
        throw XC_Socket_Exception("[XC_Socket::bind] bind '" + sServerAddr + ":" + XC_Common::tostr(port) + "' error", errno);
    }
}

void XC_Socket::bind(const char *sPathName)
{
    assert(_iDomain == AF_LOCAL);
    ...
    {
        throw XC_Socket_Exception("[XC_Socket::bind] bind '" + string(sPathName) + "' error", errno);
    }
}

void XC_Socket::bind(struct sockaddr *pstBindAddr, socklen_t iAddrLen)
{
    ...
    {
        throw XC_Socket_Exception("[XC_Socket::bind] bind error", errno);
    }
}

void XC_Socket::close()
{
...
int XC_Socket::connectNoThrow(const string &sServerAddr, uint16_t port)
{
    assert(_iDomain == AF_INET);

    if (sServerAddr == "")
    {
        throw XC_Socket_Exception("[XC_Socket::connect] server address is empty error!");
    }
...
int XC_Socket::connectNoThrow(struct sockaddr* addr)
{
	assert(_iDomain == AF_INET);
...
void XC_Socket::connect(const string &sServerAddr, uint16_t port)
{
    int ret = connectNoThrow(sServerAddr, port);

    if(ret < 0)
    {
        throw XC_SocketConnect_Exception("[XC_Socket::connect] connect error", errno);
    }
}

void XC_Socket::connect(const char *sPathName)
{
...
int XC_Socket::connectNoThrow(const char *sPathName)
{
    assert(_iDomain == AF_LOCAL);
...
int XC_Socket::connect(struct sockaddr *pstServerAddr, socklen_t serverLen)
{
    return ::connect(_sock, pstServerAddr, serverLen);

}

void XC_Socket::listen(int iConnBackLog)
{
    if (::listen(_sock, iConnBackLog) < 0)
    {
        throw XC_Socket_Exception("[XC_Socket::listen] listen error", errno);
    }
}

int XC_Socket::recv(void *pvBuf, size_t iLen, int iFlag)
{
    return ::recv(_sock, pvBuf, iLen, iFlag);
}

int XC_Socket::send(const void *pvBuf, size_t iLen, int iFlag)
{
    return ::send(_sock, pvBuf, iLen, iFlag);
}

int XC_Socket::recvfrom(void *pvBuf, size_t iLen, string &sFromAddr, uint16_t &iFromPort, int iFlags)
{
...
int XC_Socket::recvfrom(void *pvBuf, size_t iLen, struct sockaddr *pstFromAddr, socklen_t &iFromLen, int iFlags)
{
    return ::recvfrom(_sock, pvBuf, iLen, iFlags, pstFromAddr, &iFromLen);
}

int XC_Socket::sendto(const void *pvBuf, size_t iLen, const string &sToAddr, uint16_t port, int iFlags)
{
...
int XC_Socket::sendto(const void *pvBuf, size_t iLen, struct sockaddr *pstToAddr, socklen_t iToLen, int iFlags)
{
    return ::sendto(_sock, pvBuf, iLen, iFlags, pstToAddr, iToLen);
}

void XC_Socket::shutdown(int iHow)
{
    if (::shutdown(_sock, iHow) < 0)
    {
        throw XC_Socket_Exception("[XC_Socket::shutdown] shutdown error", errno);
    }
}

void XC_Socket::setblock(bool bBlock)
{
...
int XC_Socket::setSockOpt(int opt, const void *pvOptVal, socklen_t optLen, int level)
{
    return setsockopt(_sock, level, opt, pvOptVal, optLen);
}

int XC_Socket::getSockOpt(int opt, void *pvOptVal, socklen_t &optLen, int level)
{
    return getsockopt(_sock, level, opt, pvOptVal, &optLen);
}

void XC_Socket::setNoCloseWait()
{
	linger stLinger;
    ...
    {
        throw XC_Socket_Exception("[XC_Socket::setNoCloseWait] error", errno);
    }
}

void XC_Socket::setCloseWait(int delay)
{
	linger stLinger;
	stLinger.l_onoff = 1;  //在close socket调用后, 但是还有数据没发送完毕的时候容许逗留
    ...
    if(setSockOpt(SO_LINGER, (const void *)&stLinger, sizeof(linger), SOL_SOCKET) == -1)
    {
        throw XC_Socket_Exception("[XC_Socket::setCloseWait] error", errno);
    }
}

void XC_Socket::setCloseWaitDefault()
{
	linger stLinger;
	stLinger.l_onoff  = 0;
	stLinger.l_linger = 0;
    ...
    if(setSockOpt(SO_LINGER, (const void *)&stLinger, sizeof(linger), SOL_SOCKET) == -1)
    {
        throw XC_Socket_Exception("[XC_Socket::setCloseWaitDefault] error", errno);
    }
}

void XC_Socket::setTcpNoDelay()
{
    int flag = 1;

    if(setSockOpt(TCP_NODELAY, (char*)&flag, int(sizeof(int)), IPPROTO_TCP) == -1)
    {
        throw XC_Socket_Exception("[XC_Socket::setTcpNoDelay] error", errno);
    }
}

void XC_Socket::setKeepAlive()
{
    int flag = 1;
    if(setSockOpt(SO_KEEPALIVE, (char*)&flag, int(sizeof(int)), SOL_SOCKET) == -1)
    {
        throw XC_Socket_Exception("[XC_Socket::setKeepAlive] error", errno);
    }
}

void XC_Socket::setSendBufferSize(int sz)
{
    if(setSockOpt(SO_SNDBUF, (char*)&sz, int(sizeof(int)), SOL_SOCKET) == -1)
    {
        throw XC_Socket_Exception("[XC_Socket::setSendBufferSize] error", errno);
    }
}

int XC_Socket::getSendBufferSize()
{
    int sz;
    socklen_t len = sizeof(sz);
    if(getSockOpt(SO_SNDBUF, (void*)&sz, len, SOL_SOCKET) == -1 || len != sizeof(sz))
    {
        throw XC_Socket_Exception("[XC_Socket::getSendBufferSize] error", errno);
    }

    return sz;
}

void XC_Socket::setRecvBufferSize(int sz)
{
    if(setSockOpt(SO_RCVBUF, (char*)&sz, int(sizeof(int)), SOL_SOCKET) == -1)
    {
        throw XC_Socket_Exception("[XC_Socket::setRecvBufferSize] error", errno);
    }
}

int XC_Socket::getRecvBufferSize()
{
    int sz;
    socklen_t len = sizeof(sz);
    if(getSockOpt(SO_RCVBUF, (void*)&sz, len, SOL_SOCKET) == -1 || len != sizeof(sz))
    {
        throw XC_Socket_Exception("[XC_Socket::getRecvBufferSize] error", errno);
    }

    return sz;
}

void XC_Socket::setblock(int fd, bool bBlock)
{
    int val = 0;

    if ((val = fcntl(fd, F_GETFL, 0)) == -1)
    {
        throw XC_Socket_Exception("[XC_Socket::setblock] fcntl [F_GETFL] error", errno);
    }
    ...
    {
        throw XC_Socket_Exception("[XC_Socket::setblock] fcntl [F_SETFL] error", errno);
    }
}

void XC_Socket::createPipe(int fds[2], bool bBlock)
{
    if(::pipe(fds) != 0)
    {
        throw XC_Socket_Exception("[XC_Socket::createPipe] error", errno);
    }
...
vector<string> XC_Socket::getLocalHosts()
{
    ...
    while(true)
    {
        ...
        {
            free(ifc.ifc_buf);
            throw XC_Socket_Exception("[XC_Socket::getLocalHosts] ioctl error", errno);
        }
