...
#ifndef __YAC_SQueue_H__
#define __YAC_SQueue_H__
...
{
public:
	YAC_SQueue() {_header = NULL;_data = NULL;}
	~YAC_SQueue() {_header = NULL;_data = NULL;}

	void attach(char* pBuf, unsigned long iBufSize) throw (YAC_Exception)
	{
		if(iBufSize <= sizeof(Header)+MarkLen+ReserveLen) {
			throw YAC_Exception("YAC_SQueue::attach fail:iBufSize is too small");
		}

		_header = (Header *)pBuf;
		_data = pBuf+sizeof(Header);
		if(_header->iBufSize != iBufSize - sizeof(Header))
			throw YAC_Exception("YAC_SQueue::attach fail: iBufSize != iBufSize - sizeof(Header);");
		if(_header->iReserveLen != ReserveLen)
			throw YAC_Exception("YAC_SQueue::attach fail: iReserveLen != ReserveLen");
		if(_header->iBegin >= _header->iBufSize)
			throw YAC_Exception("YAC_SQueue::attach fail: iBegin > iBufSize - sizeof(Header);");
		if(_header->iEnd > iBufSize - sizeof(Header))
			throw YAC_Exception("YAC_SQueue::attach fail: iEnd > iBufSize - sizeof(Header);");
	}

	void create(char* pBuf, unsigned long iBufSize) throw (YAC_Exception)
	{
		if(iBufSize <= sizeof(Header)+MarkLen+ReserveLen) {
			throw YAC_Exception("YAC_SQueue::create fail:iBufSize is too small");
		}
