#ifndef __YAC_HTTP_H_
#define __YAC_HTTP_H_
...
#include "util/yac_timeout_queue.h"
#include <map>
#include <sstream>
#include <cassert>
#include <vector>
 ...
 * @brief  http类.  
 *  
 * 包括YAC_HttpRequest、YAC_HttpResponse两个类； 
 *  
 * 支持GET和POST，其他HTTP方法不支持； 
 *  
 * 通过YAC_HttpRequest::checkRequest判断http请求是否收完； 
 *  
 * 与YAC_ClientSocket配合，支持同步发送http请求，且支持http的chunk编码； 
 *  
 * 发送http请求时，非线程安全 
 *  
...
* @brief  http协议解析异常类
*/
struct YAC_Http_Exception : public YAC_Exception
{
    YAC_Http_Exception(const string &sBuffer) : YAC_Exception(sBuffer){};
    ~YAC_Http_Exception() throw(){};
};

/**
 * @brief  http应答协议解析异常类
 */
struct YAC_HttpResponse_Exception : public YAC_Http_Exception
{
    YAC_HttpResponse_Exception(const string &sBuffer) : YAC_Http_Exception(sBuffer){};
    ~YAC_HttpResponse_Exception() throw(){};
};

/**
 * @brief  http请求协议解析异常类
 */
struct YAC_HttpRequest_Exception : public YAC_Http_Exception
{
    YAC_HttpRequest_Exception(const string &sBuffer) : YAC_Http_Exception(sBuffer){};
    ~YAC_HttpRequest_Exception() throw(){};
};
...
public:
	 ...
	 * @return 返回调整后的URL 
     */
    YAC_URL buildWithRelativePath(const string &sRelativeURL) const;
...
public:
    ...
    struct CmpCase
    {
        ...
        {
            return YAC_Common::upper(s1) < YAC_Common::upper(s2);
        }
    };
    ...
    {
        setHeader("Content-Length", YAC_Common::tostr(iContentLength));
    }
    ...
    void setHeader(const string &sHeadName, const string &sHeadValue) 
    {
        ...
        if(YAC_Common::upper(sHeadName) != "SET-COOKIE" && YAC_Common::upper(sHeadName) != "COOKIE")
        {
 ...
 * @brief 简单的Cookie管理类.  
 */
class YAC_HttpCookie
{
public:
    typedef map<string, string, YAC_Http::CmpCase> http_cookie_data;
...
protected:
     ...
     * @return size_t
     */
    size_t isCookieMatch(const Cookie &cookie, const YAC_URL &tURL) const;
...
class YAC_HttpResponse : public YAC_Http
{
public:
     ...
     * (能够解析的数据YAC_HttpResponse会自动从sBuffer里面消除，直到网络接收完毕或者解析返回true)
     * @param buffer
     * @throws YAC_HttpResponse_Exception, 不支持的http协议, 抛出异常
...
class YAC_HttpRequest : public YAC_Http
{
public:
    ...
    YAC_HttpRequest()
    {
        YAC_HttpRequest::reset();
        setUserAgent("YAC_Http");
    }
     ...
     * @throws YAC_HttpRequest_Exception, 不支持的http协议, 抛出异常
	 ...
	 * @throw         YAC_HttpRequest_Exception 
     */
    bool decode(const string &sBuffer);
	 ...
	 * @param iTimeout 毫秒 
	 * @return        0 成功； 
	 *  			 <0失败, 具体值参见YAC_ClientSocket声明
     */
    int doRequest(YAC_HttpResponse &stHttpRep, int iTimeout = 3000);
     ...
     * @brief 获取请求的URL.
     * 
     * @return const YAC_URL&
     */
    const YAC_URL &getURL() const { return _httpURL; }
