#ifndef __YAC_MALLOC_CHUNK_H
#define __YAC_MALLOC_CHUNK_H
...
namespace util
{
	...
	* YAC_Span内存分配器
	*
	* 用于分配sizeof(YAC_Span)大小的内存块
	*/
	class YAC_SpanAllocator
	{
	public:
		YAC_SpanAllocator() : _pHead(NULL), _pData(NULL) {}
	...
	protected:
		...
		//禁止copy构造
		YAC_SpanAllocator(const YAC_SpanAllocator &mcm);

		//禁止复制
		YAC_SpanAllocator &operator=(const YAC_SpanAllocator &mcm);
	...
	*/
	...
	public:
		...
		struct YAC_CenterList
		{
			size_t       size_class;			/*内存大小的类别*/
			YAC_Span      empty;					/*空闲链表*/
			YAC_Span      nonempty;				/*非空闲链表*/
		}__attribute__((packed));

	public:
		YAC_Page() : _pShmFlagHead(NULL),_pCenterCache(NULL),_pLarge(NULL),_pFree(NULL),_pSpanMemHead(NULL),_pPageMap(NULL),_pData(NULL) {}
		 ...
		 * @param iPageId, 该内存所属的YAC_Span的起始页号
		 * @param iIndex, 该内存所属的YAC_Span的按iAllocSize大小划分后的第iIndex个
		 * @return void *
		 */
		void* fetchFromSpansSafe(size_t iClassSize, size_t &iAllocSize, size_t &iPageId, size_t &iIndex);

		/**
		 * 释放内存, 根据该内存所属YAC_Span的起始页和该YAC_Span按大小类别划分后的第iIndex个
		 * @param iPageId, 该内存所属的YAC_Span的起始页号
		 * @param iIndex, 该内存所属的YAC_Span的按iAllocSize大小划分后的第iIndex个
		 */
		...
		/**
		 * 根据该内存所属YAC_Span的起始页和该YAC_Span按大小类别划分后的第iIndex个得到该内存块的起始绝对地址
		 * @param iPageId, 该内存所属的YAC_Span的起始页号
		 * @param iIndex, 该内存所属的YAC_Span的按iAllocSize大小划分后的第iIndex个
		 */
		...
		{ 
			return sizeof(tagShmFlag) + sizeof(tagModifyHead) + sizeof(YAC_CenterList) * kNumClasses + sizeof(YAC_Span) + sizeof(YAC_Span) * kMaxPages + YAC_SpanAllocator::getHeadSize() + sizeof(YAC_Span) + sizeof(size_t) + kPageSize; 
		}

	protected:

		//禁止copy构造
		YAC_Page(const YAC_Page &mcm);

		//禁止复制
		YAC_Page &operator=(const YAC_Page &mcm);
		 ...
		 * 初始化list双向链表
		 */
		inline void DLL_Init(YAC_Span* list, size_t iIndex);

		/**
		 * 从双向链表中删除span指向的节点
		 */
		inline void DLL_Remove(YAC_Span* span);

		/**
		 * 将span指向的节点加入到双向链表list中
		 */
		inline void DLL_Prepend(YAC_Span* list, YAC_Span* span);

		/**
		 * 双向链表list是否为空
		 */
		inline bool DLL_IsEmpty(YAC_Span* list, size_t iIndex) 
		{
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////
		/**
		 * 得到PageMap中第k个元素存放的指针值，其实际意义就是看Data内存区中第k块内存页属于哪个YAC_Span管理
		 */
		...
		/**
		 * 设置PageMap中第k个元素存放的指针值，其实际意义就是看Data内存区中第k块内存页属于哪个YAC_Span管理
		 */
		...
		/**
		 * 得到内存页pPageId属于哪个YAC_Span管理
		 */
		YAC_Span* GetDescriptor(size_t pPageId);

		/**
		 * 在_pFree或_pLarge链表中查找n块连续的内存页，通过YAC_Span返回
		 */
		YAC_Span* SearchFreeAndLargeLists(size_t n);

		/**
		 * 分配n块连续的内存页，通过YAC_Span返回，里面会调用SearchFreeAndLargeLists函数
		 */
		YAC_Span* New(size_t n);

		/**
		 * 在_pLarge链表中分配n块连续的内存页，通过YAC_Span返回
		 */
		YAC_Span* AllocLarge(size_t n);

		/**
		 * 在span所管理的内存页中，分配出n块连续内存
		 */
		YAC_Span* Carve(YAC_Span* span, size_t n);

		/**
		 * 根据span所管理的内存页的数目，将该span插入到_pLarge或_pFree链表中
		 */
		void PrependToFreeList(YAC_Span* span);

		/**
		 * 将span从它所属的链表中删除，里面调用MergeIntoFreeList
		 */
		void Delete(YAC_Span* span);

		/**
		 * 将span从它所属的链表中删除，并且查看该span所管理的内存页前后连续的内存页是否空闲，若是，则进行合并操作
		 */
		void MergeIntoFreeList(YAC_Span* span);

		/**
		 * 设置span所管理的内存页将要进行分割的内存大小类别
		 */
		void RegisterSizeClass(YAC_Span* span, size_t sc);

		/**
		 * 将span所属的内存页，映射到_pPageMap中
		 */
		void RecordSpan(YAC_Span* span);
	...
	private:
		...
		YAC_Span                   *_pFree;
		/**
		 * 用于分配YAC_Span的内存区域的头指针
		 */
		YAC_SpanAllocator		  *_pSpanMemHead;
		/**
		 * 与数据区内存相映射的内存区头指针
		 */
		//void                      **_pPageMap;
		 ...
		 * 用于分配YAC_Span的内存分配器
		 */
		YAC_SpanAllocator		  _spanAlloc;
	};

	////////////////////////////////////////////////////////////////////////////////////////////////////
	class YAC_MallocChunkAllocator
	{
	public:
		YAC_MallocChunkAllocator():_pHead(NULL),_pChunk(NULL),_nallocator(NULL) {}

		~YAC_MallocChunkAllocator()
		{
			clear();
		}
		...
		/**
		 * 根据该内存所属YAC_Span的起始页和该YAC_Span按大小类别划分后的第iIndex个，换算成绝对地址
		 * @param iPageId
		 * @param iIndex
		 * @return void*
		 */
		 ...
		 */
		static size_t getNeedMinSize() { return sizeof(tagChunkAllocatorHead) + YAC_Page::getMinMemSize(); }
	protected:
		void init(void *pAddr);

		void _connect(void *pAddr);

		YAC_MallocChunkAllocator *lastAlloc();

		//禁止copy构造
		YAC_MallocChunkAllocator(const YAC_MallocChunkAllocator &);

		//禁止复制
		YAC_MallocChunkAllocator& operator=(const YAC_MallocChunkAllocator &);
	...
	private:
		 ...
		 * 后续的多块分配器
		 */
		YAC_MallocChunkAllocator *_nallocator;
	};
