...
/////////////////////////////////////////////////
class YAC_Cgi;
class YAC_Cgi_Upload;
class YAC_HttpRequest;

/**
* @brief 配置文件异常类
*/
struct YAC_Cgi_Exception : public YAC_Exception
{
	YAC_Cgi_Exception(const string &buffer) : YAC_Exception(buffer){};
    YAC_Cgi_Exception(const string &buffer, int err) : YAC_Exception(buffer, err){};
	~YAC_Cgi_Exception() throw(){};
};

/**
* @brief 全局的友元函数，定义该函数, 
*   	 则可以用YAC_Common::tostr对 vector<YAC_Cgi_Upload>进行输出操作
*/
...
{
public:
    friend ostream &operator<<(ostream &os, const YAC_Cgi_Upload &tcCgiUpload);
    ...
    */
    ...
    YAC_Cgi_Upload(const YAC_Cgi_Upload &tcCgiUpload);

    /**
    * @brief 赋值构造函数
    */
    YAC_Cgi_Upload & operator=(const YAC_Cgi_Upload &tcCgiUpload);
...
public:
    ...
    */
     ...
     */
    void parseCgi(const YAC_HttpRequest &request);
	...
	* @return          按照以上标准超过大小的返回true，否则返回false
    */
    bool  isUploadOverSize(vector<YAC_Cgi_Upload> &vtUploads) const;
	...
	* @brief 获取上传文件的相关信息
	*  
	* @return map<string,YAC_Cgi_Upload>结构中， 
	*   	  保存文件名和文件相关信息的map
    */
    const map<string, YAC_Cgi_Upload> &getUploadFilesMap() const;
...
protected:
    ...
    */
    YAC_Cgi &operator=(const YAC_Cgi &tcCgi);
...
protected:
    ...
    * 上传文件相关信息保存在该map中
    */
    map<string, YAC_Cgi_Upload>  _mpUpload;
};
