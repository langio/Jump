...
template<class T>
class YAC_TimeoutQueue: public YAC_ThreadMutex, public YAC_HandleBase
{
public:
   ...
   // typedef map<uint32_t, PtrInfo> data_type;
    ...
    typedef YAC_Functor<void, TL::TYPELIST_1(T&)> data_functor;
     ...
     * @param size
     */
    YAC_TimeoutQueue(int timeout = 5*1000,size_t size = 100 ) : _uniqId(0), _timeout(timeout) {_data.resize(size);}
     ...
     * @return size_t
     */
    size_t size() const { YAC_LockT<YAC_ThreadMutex> lock(*this); return _data.size(); }
...
template<typename T> T YAC_TimeoutQueue<T>::get(uint32_t uniqId, bool bErase)
{
...
template<typename T> uint32_t YAC_TimeoutQueue<T>::generateId()
{
    YAC_LockT<YAC_ThreadMutex> lock(*this);
...
template<typename T> bool YAC_TimeoutQueue<T>::push(T& ptr, uint32_t uniqId)
{
    YAC_LockT<YAC_ThreadMutex> lock(*this);
...
template<typename T> void YAC_TimeoutQueue<T>::timeout()
{
    struct timeval tv;
    YAC_TimeProvider::getInstance()->getNow(&tv);
...
template<typename T> void YAC_TimeoutQueue<T>::timeout(data_functor &df)
{
    struct timeval tv;
    YAC_TimeProvider::getInstance()->getNow(&tv);
    while(true)
...
template<typename T> T YAC_TimeoutQueue<T>::erase(uint32_t uniqId)
{
    YAC_LockT<YAC_ThreadMutex> lock(*this);
...
template<typename T> T YAC_TimeoutQueue<T>::pop()
{
    T ptr;
...
template<typename T> bool YAC_TimeoutQueue<T>::pop(T &ptr)
{
    YAC_LockT<YAC_ThreadMutex> lock(*this);
...
template<typename T> bool YAC_TimeoutQueue<T>::swap(deque<T> &q)
{
    YAC_LockT<YAC_ThreadMutex> lock(*this);
