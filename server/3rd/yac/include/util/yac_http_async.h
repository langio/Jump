...
* @brief 线程异常
*/
struct YAC_HttpAsync_Exception : public YAC_Exception
{
    YAC_HttpAsync_Exception(const string &buffer) : YAC_Exception(buffer){};
    YAC_HttpAsync_Exception(const string &buffer, int err) : YAC_Exception(buffer, err){};
    ~YAC_HttpAsync_Exception() throw(){};
};
 ...
 *    实现异步回调对象
 *  
  *  异步对象回调执行的时候是在YAC_HttpAsync中线程执行的
 *  
 *   用智能指针new出来, 不用管生命周期
 *  
 *   class AsyncHttpCallback : public
 *  
 *    YAC_HttpAsync::RequestCallback
 *  
 *  {
 *  
 *   public:
 ...
 *  
  ...
  * 	
 *   virtual void onResponse(bool bClose, YAC_HttpResponse
 *  
  * 	 &stHttpResponse)
 *  
  *  	 {
 ...
 *  
  *  		YAC_Common::tostr(stHttpResponse.getHeaders()) <<
 *  
  *  		endl;
  *  	}
  *  
 ...
 *   //封装一个函数, 根据实际情况处理
 *  
 *  int addAsyncRequest(YAC_HttpAsync &ast, const string &sUrl) {
 *  
 *  	YAC_HttpRequest stHttpReq; stHttpReq.setGetRequest(sUrl);
 *  
  *
  *      //new出来一个异步回调对象
 *  
  * 	 YAC_HttpAsync::RequestCallbackPtr p = new
 *  
 ...
 *   ast.terminate(); 
 */
class YAC_HttpAsync : public YAC_Thread, public YAC_ThreadLock
{
public:
    /**
     * @brief 异步请求回调对象
     */
    class RequestCallback : public YAC_HandleBase
    {
    public:
        /**
		 * @brief 完整的响应回来了. 
         ...
         */
        virtual void onResponse(bool bClose, YAC_HttpResponse &stHttpResponse) = 0;
         ...
         */
        virtual bool onReceive(YAC_HttpResponse &stHttpResponse) { return true;};
    ...
    typedef YAC_AutoPtr<RequestCallback> RequestCallbackPtr;

protected:
    /**
     * @brief 异步http请求(短连接)
     */
    class AsyncRequest : public YAC_HandleBase
    {
    public:
        /**
		 * @brief 构造. 
         ...
         * @param callbackPtr
         */
        AsyncRequest(YAC_HttpRequest &stHttpRequest, RequestCallbackPtr &callbackPtr);
         ...
         */
        void setHttpAsync(YAC_HttpAsync *pHttpAsync) { _pHttpAsync = pHttpAsync; }
    ...
    protected:
        YAC_HttpAsync               *_pHttpAsync;
        YAC_HttpResponse             _stHttpResp;
        YAC_Socket                   _fd;
        string                      _sHost;
        uint32_t                    _iPort;
        uint32_t                    _iUniqId;
    ...
    typedef YAC_AutoPtr<AsyncRequest> AsyncRequestPtr;

public:

    typedef YAC_TimeoutQueue<AsyncRequestPtr> http_queue_type;
     ...
     *             =0:成功
     */
    int doAsyncRequest(YAC_HttpRequest &stHttpRequest, RequestCallbackPtr &callbackPtr, bool bUseProxy=false, struct sockaddr* addr=NULL);
...
protected:
    ...
    typedef YAC_Functor<void, TL::TLMaker<AsyncRequestPtr, int>::Result> async_process_type;
