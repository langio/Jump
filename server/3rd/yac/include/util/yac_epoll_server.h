#ifndef __YAC_EPOLL_SERVER_H_
#define __YAC_EPOLL_SERVER_H_
...
#include "util/yac_mem_queue.h"
#include "util/yac_squeue.h"
 ...
 * 注册管理端口处理器
 */
class YAC_EpollServer : public YAC_ThreadLock, public YAC_HandleBase
{
public:
    //////////////////////////////////////////////////////////////
    class BindAdapter;
    typedef YAC_AutoPtr<BindAdapter> BindAdapterPtr;

    class Handle;
    typedef YAC_AutoPtr<Handle> HandlePtr;

    class HandleGroup;
    typedef YAC_AutoPtr<HandleGroup> HandleGroupPtr;

    /**
     * 定义协议解析接口的操作对象
     * 注意必须是线程安全的或是可以重入的
     */
    typedef YAC_Functor<int, TL::TLMaker<string &, string&>::Result> protocol_functor;
    typedef YAC_Functor<int, TL::TLMaker<int, string&>::Result>      header_filter_functor;
    //////////////////////////////////////////////////////////////
    /**定义数据队列中的结构*/

    struct tagRecvData: public YAC_HandleBase
    {
        uint32_t        uid;            /**连接标示*/
    ...
    //typedef YAC_AutoPtr<tagRecvData> tagRecvDataPtr;

    struct tagSendData: public YAC_HandleBase
    {
    ...
    //typedef YAC_AutoPtr<tagSendData> tagSendDataPtr;

    typedef YAC_ThreadQueue<tagRecvData*, deque<tagRecvData*> > recv_queue;
    typedef YAC_ThreadQueue<tagSendData*, deque<tagSendData*> > send_queue;
    typedef recv_queue::queue_type recv_queue_type;


    class Adapter_ShmQueue : public YAC_ThreadLock
    {
    public:
        Adapter_ShmQueue();
     ...
     * 每个handle对象一个线程
     */
    struct HandleGroup : public YAC_HandleBase
    {
        string                      name;
        YAC_ThreadLock               monitor;
     ...
     * 服务的逻辑处理代码
     */
    class Handle : public YAC_Thread, public YAC_ThreadLock, public YAC_HandleBase
    {
    public:
        /**
         * 构造, 默认没有请求, 等待10s
         ...
         * @param pEpollServer
         */
        void setEpollServer(YAC_EpollServer *pEpollServer);

        /**
         * 获取服务
         * @return YAC_EpollServer*
         */
        YAC_EpollServer* getEpollServer();
    ...
    //////////////////////////////////////////////////////////////////////////////////
    // 服务端口管理,监听socket信息
    class BindAdapter : public YAC_ThreadLock, public YAC_HandleBase
    {
    public:
        /**
         * 缺省的一些定义
        ...
        struct BusCommu
        {
            ...
            YAC_SQueue c2sMemQueue;
            YAC_Mmap     c2sMmap;
            YAC_Fifo     c2sFifo;
            
            YAC_SQueue s2cMemQueue;
            YAC_Mmap     s2cMmap;
            YAC_Fifo     s2cFifo;
        };
        /**
         ...
         * @param ppPtr, 协议解析类
         */
        BindAdapter(YAC_EpollServer *pEpollServer);
         ...
         * @return const string&
         */
        YAC_Endpoint getEndpoint() const;

        /**
         * 监听socket
         * @return YAC_Socket
         */
        YAC_Socket &getSocket();
     ...
     * 带有时间链表的map
     */
    class ConnectionList : public YAC_ThreadLock
    {
    public:
         ...
         * 构造函数
         */
        ConnectionList(YAC_EpollServer *pEpollServer) : _pEpollServer(pEpollServer), _vConn(NULL) {}
         ...
         * 获取某个监听端口的连接
         * @param lfd
         * @return vector<YAC_EpollServer::ConnStatus>
         */
        vector<YAC_EpollServer::ConnStatus> getConnStatus(int lfd);
...
public:
     ...
     * @return YAC_Epoller*
     */
    YAC_Epoller* getEpoller()    { return &_epoller; }
...
protected:
     ...
     * @param s
     */
    void bind(const YAC_Endpoint &ep, YAC_Socket &s);
...
typedef YAC_AutoPtr<YAC_EpollServer> YAC_EpollServerPtr;
////////////////////////////////////////////////////////////////////////////////
}
