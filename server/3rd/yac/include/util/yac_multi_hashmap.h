#ifndef	__YAC_MULTI_HASHMAP_H__
#define __YAC_MULTI_HASHMAP_H__
...
#include "util/yac_mem_vector.h"
#include "util/yac_pack.h"
#include "util/yac_mem_chunk.h"
#include "util/yac_functor.h"
#include "util/yac_hash_fun.h"

namespace util
{
/////////////////////////////////////////////////
/** 
 * @file yac_multi_hashmap.h  
...
*/
struct YAC_Multi_HashMap_Exception : public YAC_Exception
{
	YAC_Multi_HashMap_Exception(const string &buffer) : YAC_Exception(buffer){};
    YAC_Multi_HashMap_Exception(const string &buffer, int err) : YAC_Exception(buffer, err){};
    ~YAC_Multi_HashMap_Exception() throw(){};
};
...
public:
	...
	*/
	...
	public:
         ...
         * @param iAddr, 主key的地址
         */
        MainKey(YAC_Multi_HashMap *pMap, uint32_t iAddr)
        : _pMap(pMap)
        , _iHead(iAddr)
        {
			// 构造时保存绝对地址，避免每次都计算
         ...
         * @param vtData, 淘汰的数据
         */
        int set(const void *pData, uint32_t iDataLen, vector<YAC_Multi_HashMap::Value> &vtData);
         ...
         * 返回被删除的主key下的所有数据
         * @return int, YAC_Multi_HashMap::RT_OK成功，其它失败
         */
    ...
    protected:
         ...
         * @return int
         */
        int allocate(uint32_t iDataLen, vector<YAC_Multi_HashMap::Value> &vtData);
         ...
         * @return int
         */
        int allocateChunk(uint32_t fn, vector<uint32_t> &chunks, vector<YAC_Multi_HashMap::Value> &vtData);
	...
	public:
		bool ISFULLDATA()						{ return YAC_Multi_HashMap::ISSET(getHeadPtr()->_iBitset, INTEGRITY_BIT); }
	protected:
		bool HASNEXTCHUNK()						{ return YAC_Multi_HashMap::ISSET(getHeadPtr()->_iBitset, NEXTCHUNK_BIT); }
		
	public:
		void SETFULLDATA(bool b)				{ if(b) YAC_Multi_HashMap::SET(getHeadPtr()->_iBitset, INTEGRITY_BIT); else YAC_Multi_HashMap::UNSET(getHeadPtr()->_iBitset, INTEGRITY_BIT); }
	protected:
		void SETNEXTCHUNK(bool b)				{ if(b) YAC_Multi_HashMap::SET(getHeadPtr()->_iBitset, NEXTCHUNK_BIT); else YAC_Multi_HashMap::UNSET(getHeadPtr()->_iBitset, NEXTCHUNK_BIT); }
    ...
    */
    ...
    public:
		...
		*/
         ...
         * @param pAdd
         */
        Block(YAC_Multi_HashMap *pMap, uint32_t iAddr)
        : _pMap(pMap)
        , _iHead(iAddr)
        {
			// 构造时保存绝对地址，避免每次都计算
		...
		*/
         ...
         * @return int
         *          YAC_Multi_HashMap::RT_OK, 正常, 其他异常
         *          YAC_Multi_HashMap::RT_ONLY_KEY, 只有Key
         *          其他异常
         */
        int getBlockData(YAC_Multi_HashMap::BlockData &data);
		 ...
		 *				其它为失败
         */
        int set(const void *pData, uint32_t iDataLen, bool bOnlyKey, uint8_t iVersion, vector<YAC_Multi_HashMap::Value> &vtData);
    ...
    protected:
		...
		bool HASNEXTCHUNK()						{ return YAC_Multi_HashMap::ISSET(getBlockHead()->_iBitset, NEXTCHUNK_BIT); }
		bool ISDIRTY()							{ return YAC_Multi_HashMap::ISSET(getBlockHead()->_iBitset, DIRTY_BIT); }
		bool ISONLYKEY()						{ return YAC_Multi_HashMap::ISSET(getBlockHead()->_iBitset, ONLYKEY_BIT); }

		void SETNEXTCHUNK(bool b)				{ if(b) YAC_Multi_HashMap::SET(getBlockHead()->_iBitset, NEXTCHUNK_BIT); else YAC_Multi_HashMap::UNSET(getBlockHead()->_iBitset, NEXTCHUNK_BIT); }
		void SETDIRTY(bool b)					{ if(b) YAC_Multi_HashMap::SET(getBlockHead()->_iBitset, DIRTY_BIT); else YAC_Multi_HashMap::UNSET(getBlockHead()->_iBitset, DIRTY_BIT); }
		void SETONLYKEY(bool b)					{ if(b) YAC_Multi_HashMap::SET(getBlockHead()->_iBitset, ONLYKEY_BIT); else YAC_Multi_HashMap::UNSET(getBlockHead()->_iBitset, ONLYKEY_BIT); }
    ...
    */
    ...
    public:
         ...
         * @brief 构造函数
         */
        BlockAllocator(YAC_Multi_HashMap *pMap)
        : _pMap(pMap)
        , _pChunkAllocator(new YAC_MemMultiChunkAllocator())
        {
        }
         ...
         * @brief 获取每种数据块头部信息
         *
         * @return vector<YAC_MemChunk::tagChunkHead>
         */
        vector<YAC_MemChunk::tagChunkHead> getBlockDetail() const  { return _pChunkAllocator->getBlockDetail(); }
         ...
         */
        uint32_t allocateMemBlock(uint32_t iMainKeyAddr, uint32_t index, bool bHead, uint32_t &iAllocSize, vector<YAC_Multi_HashMap::Value> &vtData);
		...
		*/
		uint32_t allocateMainKeyHead(uint32_t index, vector<YAC_Multi_HashMap::Value> &vtData);
         ...
         */
        uint32_t allocateChunk(uint32_t iAddr, uint32_t &iAllocSize, vector<YAC_Multi_HashMap::Value> &vtData);
    ...
    public:
         ...
         * chunk分配器
         */
        YAC_MemMultiChunkAllocator		*_pChunkAllocator;
    };
    ...
    public:
    	 ...
    	 */
    	HashMapLockItem(YAC_Multi_HashMap *pMap, uint32_t iAddr);
         ...
         *
    	 */
		int get(YAC_Multi_HashMap::Value &v);
    ...
    protected:
         ...
         * @return int
         */
        int set(const string &mk, const string &uk, const string& v, uint8_t iVersion, vector<YAC_Multi_HashMap::Value> &vtData);
         ...
         * @return int
         */
        int set(const string &mk, const string &uk, vector<YAC_Multi_HashMap::Value> &vtData);
         ...
         * @return bool
         */
        bool equal(const string &mk, const string &uk, YAC_Multi_HashMap::Value &v, int &ret);
        ...
        friend class YAC_Multi_HashMap;
        friend struct YAC_Multi_HashMap::HashMapLockIterator;
    ...
    public:
		 ...
		 * @param iOrder, 遍历的顺序
    	 */
    	HashMapLockIterator(YAC_Multi_HashMap *pMap, uint32_t iAddr, int iType, int iOrder);
    ...
    public:
         ...
         * @param iIndex, Hash索引
         */
        HashMapItem(YAC_Multi_HashMap *pMap, uint32_t iIndex);
         ...
         * @return
         */
        void get(vector<YAC_Multi_HashMap::Value> &vtData);
        ...
        friend class YAC_Multi_HashMap;
        friend struct YAC_Multi_HashMap::HashMapIterator;
	...
	*/
    ...
    public:
    	 ...
    	 * @param type
    	 */
    	HashMapIterator(YAC_Multi_HashMap *pMap, uint32_t iIndex);
	...
	*/
    ...
    /**定义hash处理器*/
    typedef YAC_Functor<uint32_t, TL::TLMaker<const string &>::Result> hash_functor;
     ...
     *
     * @return vector<YAC_MemChunk::tagChunkHead>: 不同大小内存块头部信息
     */
    vector<YAC_MemChunk::tagChunkHead> getBlockDetail() { return _pDataAllocator->getBlockDetail(); }
	...
	*/
    ...
    /**
     * @brief 设置淘汰方式
     * YAC_Multi_HashMap::ERASEBYGET
     * YAC_Multi_HashMap::ERASEBYSET
     * @param cEraseMode
     */
	...
	*
	* @return int
	*		YAC_Multi_HashMap::RT_OK, 主key存在，且有数据
	*		YAC_Multi_HashMap::RT_ONLY_KEY, 主key存在，没有数据
	*		YAC_Multi_HashMap::RT_PART_DATA, 主key存在，里面的数据可能不完整
	*		YAC_Multi_HashMap::RT_NO_DATA, 主key不存在
	*/
...
protected:
	...
	*  @brief 禁止copy构造
	*/
    YAC_Multi_HashMap(const YAC_Multi_HashMap &mcm);
   	/**
	*  @brief 禁止复制
	*/
    YAC_Multi_HashMap &operator=(const YAC_Multi_HashMap &mcm);
	...
	*/
    ...
    {
        FailureRecover(YAC_Multi_HashMap *pMap) : _pMap(pMap)
        {
			// 构造时恢复可能损坏的数据
            _pMap->doRecover();
			assert(_iRefCount ++ == 0);
...
protected:
	...
	*/
     ...
     * 联合主键hash索引区
     */
    YAC_MemVector<tagHashItem>   _hash;

	/**
	* 主key hash索引区
	*/
	YAC_MemVector<tagMainKeyHashItem>	_hashMainKey;
