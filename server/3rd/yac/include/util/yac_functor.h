#ifndef __YAC_FUNCTOR_H
#define __YAC_FUNCTOR_H

#include <memory>
#include <iostream>
#include "util/yac_loki.h"
 ...
 *  }
 *
 * YAC_Functor<void, TL::TLMaker<const string&, int>::Result> cmd3(TestFunction3);
 * string s3("s3");
 * cmd3(s3, 10);
 * C函数调用用YAC_FunctorWrapper封装:*
 *  YAC_FunctorWrapper<YAC_Functor<void, TL::TLMaker<const string&, int>::Result> > fwrapper3(cmd3, s3, 10);
 *  fwrapper3();
 * 说明:
 * void : 函数的返回值
 * C++函数对象调用:
 ...
 *  TestFunctor f;
 *
 *  YAC_Functor<void, TL::TLMaker<const string&, int>::Result> cmd3(f);
 *
 *  string s3("s3");
 *
 *  cmd3(s3, 10);
 *
 * C++函数对象调用用YAC_FunctorWrapper封装:
 *
 *  YAC_FunctorWrapper<YAC_Functor<void, TL::TLMaker<const string&, int>::Result> > fwrapper3(cmd3, s3, 10);
 *
 *  fwrapper3();
 ...
 *     }
 *  }
 *  YAC_Functor<void, TL::TLMaker<const string&, int>::Result> cmd3(&tm, &TestMember::mem3);
 * cmd3("a", 33);
 * 指向类成员函数的调用用YAC_FunctorWrapper封装:
 *  YAC_FunctorWrapper<YAC_Functor<void, TL::TLMaker<const string&, int>::Result> > fwrapper3(cmd3, "a", 10);
 *  fwrapper3();
 * 注意所有的YAC_FunctorWrapper的调用, 在具体调用的时候是不带参数的, 参数在构造的时候已经传进去了
 ///////////////////////////////////////////////////////////////////////////////////////////////////
 *
 * 对于C风格的函数调用, 同名称的函数通常只有一个, 如果有多个相同名称的函数
 *不同参数类型的函数, 则在构造YAC_Functor的时候需要强制类型转换, 如下:
 *  void TestFunction1()
 *  {
 ...
 * }
 * typedef void (*TpFunc)();
 * YAC_Functor<void> cmd1(static_cast<TpFunc>(TestFunction1));
 * cmd1();
 * typedef void (*TpFunc1)(char);
 * YAC_Functor<void, TL::TLMaker<char>::Result>
 ...
 *  YAC_Functor<void> cmd1(&tm, static_cast<TpMem>(&TestMember::mem1));
 *  cmd1();
 *
 *  typedef void (TestMember::*TpMem1)(char);
 
 * YAC_Functor<void, TL::TLMaker<char>::Result> cmd11(&tm,
...
class YAC_FunctorImp
{
public:
    virtual R operator()() = 0;
    virtual YAC_FunctorImp* clone() const = 0;
    virtual ~YAC_FunctorImp() {};
};

template<typename R>
class YAC_FunctorImp<R, TL::EmptyType>
{
public:
    virtual R operator()() = 0;
    virtual YAC_FunctorImp* clone() const = 0;
...
class YAC_FunctorImp<R, TL::TYPELIST_1(P1)>
{
public:
...
class YAC_FunctorImp<R, TL::TYPELIST_2(P1, P2)>
{
public:
...
class YAC_FunctorImp<R, TL::TYPELIST_3(P1, P2, P3)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
...
class YAC_FunctorImp<R, TL::TYPELIST_4(P1, P2, P3, P4)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
                         typename TL::TypeTraits<P2>::ReferenceType p2,
...
class YAC_FunctorImp<R, TL::TYPELIST_5(P1, P2, P3, P4, P5)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
                         typename TL::TypeTraits<P2>::ReferenceType p2,
...
class YAC_FunctorImp<R, TL::TYPELIST_6(P1, P2, P3, P4, P5, P6)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
                         typename TL::TypeTraits<P2>::ReferenceType p2,
...
class YAC_FunctorImp<R, TL::TYPELIST_7(P1, P2, P3, P4, P5, P6, P7)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
                         typename TL::TypeTraits<P2>::ReferenceType p2,
...
class YAC_FunctorImp<R, TL::TYPELIST_8(P1, P2, P3, P4, P5, P6, P7, P8)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
                         typename TL::TypeTraits<P2>::ReferenceType p2,
...
class YAC_FunctorImp<R, TL::TYPELIST_9(P1, P2, P3, P4, P5, P6, P7, P8, P9)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
                         typename TL::TypeTraits<P2>::ReferenceType p2,
...
class YAC_FunctorImp<R, TL::TYPELIST_10(P1, P2, P3, P4, P5, P6, P7, P8, P9, P10)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
                         typename TL::TypeTraits<P2>::ReferenceType p2,
...
class YAC_FunctorHandler : public YAC_FunctorImp<typename ParentFunctor::ResultType, typename ParentFunctor::ParamList>
{
public:
    typedef typename ParentFunctor::ResultType ResultType;

    YAC_FunctorHandler(Fun fun) : _fun(fun)
    {

    }

    YAC_FunctorHandler *clone() const
    {
        return new YAC_FunctorHandler(*this);
    }
 ...
 * @brief 封装对成员函数的调用
 */
class YAC_MemFunHandler
	: public YAC_FunctorImp<typename ParentFunctor::ResultType, typename ParentFunctor::ParamList>
{
public:
	typedef typename ParentFunctor::ResultType ResultType;

	YAC_MemFunHandler(const PointerToObj &pObj, PointerToMemFun pMemFn)
		: _pObj(pObj), _pMemFn(pMemFn)
	{
	}

	YAC_MemFunHandler* clone() const
	{
		return new YAC_MemFunHandler(*this);
	}
...
public:
    ...
    /**定义封装类型*/
    typedef YAC_FunctorWrapper<YAC_Functor<R, TList> > wrapper_type;
...
public:
    ...
    YAC_Functor(const YAC_Functor &functor)
    : _spImpl(functor._spImpl->clone())
    {
    }

    YAC_Functor& operator=(const YAC_Functor &functor)
    {
        if(this != &functor)
        {
    ...
    template<class Fun>
    YAC_Functor(Fun fun)
	: _spImpl(new YAC_FunctorHandler<YAC_Functor, Fun>(fun))
    {
    }

	template<typename PointerToObj, typename PointerToMemFun>
	YAC_Functor(const PointerToObj &pObj, PointerToMemFun pMemFn)
	: _spImpl(new YAC_MemFunHandler<YAC_Functor, PointerToObj, PointerToMemFun>(pObj, pMemFn))
	{
	}
...
private:
    template<class ParentFunctor>
    friend class YAC_FunctorWrapper;

    typedef YAC_FunctorImp<R, TList> Impl;
 ...
 * @brief wapper基类
 */
class YAC_FunctorWrapperInterface
{
public:
    virtual void operator()() = 0;
    virtual ~YAC_FunctorWrapperInterface(){}
};

template<class ParentFunctor>

/**
 * @brief 对YAC_Functor进行封装, 可以让YAC_Functor做到事后被调用
 */
class YAC_FunctorWrapper : public YAC_FunctorWrapperInterface
{
public:
     ...
     * @param fw
     */
    YAC_FunctorWrapper(const YAC_FunctorWrapper &fw)
     ...
     * @param tf
     */
    YAC_FunctorWrapper(ParentFunctor &tf)
...
protected:
     ...
     * @param fw
     *
     * @return YAC_FunctorWrapper&
     */
    YAC_FunctorWrapper& operator=(const YAC_FunctorWrapper &fw);
