#ifndef __YAC_MYSQL_H
#define __YAC_MYSQL_H
...
* @brief 数据库异常类
*/
struct YAC_Mysql_Exception : public YAC_Exception
{
	YAC_Mysql_Exception(const string &sBuffer) : YAC_Exception(sBuffer){};
	~YAC_Mysql_Exception() throw(){};    
};
...
* @brief Mysql数据库操作类 
* 
* 非线程安全，通常一个线程一个YAC_Mysql对象；
* 
* 对于insert/update可以有更好的函数封装，保证SQL注入；
* 
* YAC_Mysql::DB_INT表示组装sql语句时，不加””和转义；
* 
* YAC_Mysql::DB_STR表示组装sql语句时，加””并转义；
*/
class YAC_Mysql 
{
public:
    ...
    * @param iFlag        客户端标识
    */
    YAC_Mysql(const string& sHost, const string& sUser = "", const string& sPasswd = "", const string& sDatabase = "", const string &sCharSet = "", int port = 0, int iFlag = 0);

    /**
    * @brief 构造函数. 
    * @param tcDBConf 数据库配置
    */
    YAC_Mysql(const YAC_DBConf& tcDBConf);
    ...
    * @param tcDBConf 数据库配置
    */
    void init(const YAC_DBConf& tcDBConf);

    /**
	* @brief 连接数据库. 
	*  
    * @throws YAC_Mysql_Exception
    * @return 无
    */
    void connect();
...
protected:
    ...
    */
    YAC_Mysql(const YAC_Mysql &tcMysql);

    /**
    * 
    * @brief 只申明,不定义,保证不被使用
    */
    YAC_Mysql &operator=(const YAC_Mysql &tcMysql);
