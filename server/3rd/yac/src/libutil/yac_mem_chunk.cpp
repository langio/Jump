...
YAC_MemChunk::YAC_MemChunk()
: _pHead(NULL)
, _pData(NULL)
{
}

size_t YAC_MemChunk::calcMemSize(size_t iBlockSize, size_t iBlockCount)
{
    return iBlockSize * iBlockCount + sizeof(tagChunkHead);
}

size_t YAC_MemChunk::calcBlockCount(size_t iMemSize, size_t iBlockSize)
{
...
void YAC_MemChunk::create(void *pAddr, size_t iBlockSize, size_t iBlockCount)
{
...
void YAC_MemChunk::connect(void *pAddr)
{
    init(pAddr);
}

void YAC_MemChunk::init(void *pAddr)
{
    _pHead = static_cast<tagChunkHead*>(pAddr);
    _pData = (unsigned char*)((char*)_pHead + sizeof(tagChunkHead));
}

void* YAC_MemChunk::allocate()
{
...
void* YAC_MemChunk::allocate2(size_t &iIndex)
{
...
void YAC_MemChunk::deallocate(void *pAddr)
{
    assert(pAddr >= _pData);
...
void YAC_MemChunk::deallocate2(size_t iIndex)
{
...
void* YAC_MemChunk::getAbsolute(size_t iIndex)
{
	assert(iIndex > 0 && iIndex <= _pHead->_iBlockCount);
...
size_t YAC_MemChunk::getRelative(void *pAddr)
{
...
YAC_MemChunk::tagChunkHead YAC_MemChunk::getChunkHead() const
{
    return *_pHead;
}
...
YAC_MemChunkAllocator::YAC_MemChunkAllocator()
: _pHead(NULL)
{
}

void YAC_MemChunkAllocator::init(void *pAddr)
{
    _pHead  = static_cast<tagChunkAllocatorHead*>(pAddr);
    _pChunk = (char*)_pHead + sizeof(tagChunkAllocatorHead);
}

void YAC_MemChunkAllocator::initChunk()
{
    assert(_pHead->_iSize > sizeof(tagChunkAllocatorHead));

    size_t iChunkCapacity = _pHead->_iSize - sizeof(tagChunkAllocatorHead);

    assert(iChunkCapacity > YAC_MemChunk::getHeadSize());

    size_t  iBlockCount   = (iChunkCapacity - YAC_MemChunk::getHeadSize()) / _pHead->_iBlockSize;
...
void YAC_MemChunkAllocator::create(void *pAddr, size_t iSize, size_t iBlockSize)
{
    init(pAddr);
...
void YAC_MemChunkAllocator::connectChunk()
{
    assert(_pHead->_iSize > sizeof(tagChunkAllocatorHead));
...
void YAC_MemChunkAllocator::connect(void *pAddr)
{
    init(pAddr);
...
void YAC_MemChunkAllocator::rebuild()
{
    _chunk.rebuild();
}

void* YAC_MemChunkAllocator::allocate()
{
    return _chunk.allocate();
}

void* YAC_MemChunkAllocator::allocate2(size_t &iIndex)
{
	return _chunk.allocate2(iIndex);
}

void YAC_MemChunkAllocator::deallocate(void *pAddr)
{
...
void YAC_MemChunkAllocator::deallocate2(size_t iIndex)
{
	_chunk.deallocate2(iIndex);
}

YAC_MemChunk::tagChunkHead YAC_MemChunkAllocator::getBlockDetail() const
{
...
/////////////////////////////////////////////////////////////////////////////////////////////////
//
YAC_MemMultiChunkAllocator::YAC_MemMultiChunkAllocator()
...
void YAC_MemMultiChunkAllocator::clear()
{
...
vector<size_t> YAC_MemMultiChunkAllocator::getBlockSize()  const    
{ 
...
size_t YAC_MemMultiChunkAllocator::getCapacity() const
{
    size_t iCapacity = 0;
    for(size_t i = 0; i < _allocator.size(); i++)
    {
...
vector<YAC_MemChunk::tagChunkHead> YAC_MemMultiChunkAllocator::getBlockDetail() const
{
    vector<YAC_MemChunk::tagChunkHead> vt;
    for(size_t i = 0; i < _allocator.size(); i++)
    {
    ...
    if(_nallocator)
    {
        vector<YAC_MemChunk::tagChunkHead> v = _nallocator->getBlockDetail();
...
vector<size_t> YAC_MemMultiChunkAllocator::singleBlockChunkCount() const                
{ 
    vector<size_t> vv;
...
size_t YAC_MemMultiChunkAllocator::allBlockChunkCount() const                   
{ 
...
void YAC_MemMultiChunkAllocator::init(void *pAddr)
{
    _pHead  = static_cast<tagChunkAllocatorHead*>(pAddr);
    _pChunk = (char*)_pHead + sizeof(tagChunkAllocatorHead);
}

void YAC_MemMultiChunkAllocator::calc()
{
    _vBlockSize.clear();
    ...
    assert(_pHead->_iSize > (YAC_MemMultiChunkAllocator::getHeadSize() + YAC_MemChunkAllocator::getHeadSize()*_vBlockSize.size() + YAC_MemChunk::getHeadSize()*_vBlockSize.size()));

    //计算块的个数
    _iBlockCount = (_pHead->_iSize
                    - YAC_MemMultiChunkAllocator::getHeadSize()
                    - YAC_MemChunkAllocator::getHeadSize() * _vBlockSize.size()
                    - YAC_MemChunk::getHeadSize() * _vBlockSize.size())/sum;

    assert(_iBlockCount >= 1);
}

void YAC_MemMultiChunkAllocator::create(void *pAddr, size_t iSize, size_t iMinBlockSize, size_t iMaxBlockSize, float fFactor)
{
    ...
    {
        YAC_MemChunkAllocator *p = new YAC_MemChunkAllocator;
        size_t iAllocSize = YAC_MemChunkAllocator::getHeadSize() + YAC_MemChunk::calcMemSize(_vBlockSize[i], _iBlockCount);
...
void YAC_MemMultiChunkAllocator::connect(void *pAddr)
{
    clear();
    ...
    for(size_t i = 0; i < _vBlockSize.size(); i++)
    {
        ...
        pChunkBegin += YAC_MemChunkAllocator::getHeadSize() + YAC_MemChunk::calcMemSize(_vBlockSize[i], _iBlockCount);
        _allocator.push_back(p);
    }
    ...
    _nallocator = new YAC_MemMultiChunkAllocator();
    _nallocator->connect(pNextHead);
}

void YAC_MemMultiChunkAllocator::rebuild()
{
    for(size_t i = 0; i < _allocator.size(); i++)
    {
...
YAC_MemMultiChunkAllocator *YAC_MemMultiChunkAllocator::lastAlloc()
{
    if(_nallocator == NULL)
        return NULL;

    YAC_MemMultiChunkAllocator *p = _nallocator;
...
void YAC_MemMultiChunkAllocator::append(void *pAddr, size_t iSize)
{
    connect(pAddr);
...
void* YAC_MemMultiChunkAllocator::allocate(size_t iNeedSize, size_t &iAllocSize)
{
    size_t iIndex;
    return allocate2(iNeedSize, iAllocSize, iIndex);
}

void *YAC_MemMultiChunkAllocator::allocate2(size_t iNeedSize, size_t &iAllocSize, size_t &iIndex)
{
    void *p = NULL;
    iIndex  = 0;
...
void YAC_MemMultiChunkAllocator::deallocate(void *pAddr)
{
...
void YAC_MemMultiChunkAllocator::deallocate2(size_t iIndex)
{
	for(size_t i = 0; i < _allocator.size(); i++)
	{
...
void* YAC_MemMultiChunkAllocator::getAbsolute(size_t iIndex)
{
	if(iIndex == 0)
	{
		return NULL;
...
size_t YAC_MemMultiChunkAllocator::getRelative(void *pAddr)
{
	if(pAddr == NULL)
	{
		return 0;
