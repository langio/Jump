...
void YAC_MemQueue::create(void *pAddr, size_t iSize)
{
    assert(pAddr);
    assert(iSize != 0);
...
void YAC_MemQueue::connect(void *pAddr, size_t iSize)
{
    assert(pAddr);
...
bool YAC_MemQueue::isFull(size_t iSize)
{
    doUpdate();
...
bool YAC_MemQueue::pop_front(string &sOut)
{
    if(isEmpty())
    {
        return false;
...
bool YAC_MemQueue::push_back(const string &sIn)
{
    return push_back(sIn.c_str(),sIn.length());
}

bool YAC_MemQueue::push_back(const char *pvIn, size_t iSize)
{
    if(isFull(iSize))
    {
        return false;
...
void YAC_MemQueue::doUpdate(bool bUpdate)
{
	if(bUpdate)
	{
...
void YAC_MemQueue::update(void* iModifyAddr, size_t iModifyValue)
{
...
void YAC_MemQueue::update(void* iModifyAddr, bool bModifyValue)
{
	_pstModifyHead->_cModifyStatus = 1;
	_pstModifyHead->_stModifyData[_pstModifyHead->_iNowIndex]._iModifyAddr  = (char*)iModifyAddr - (char*)_pctrlBlock;
