#include "util/yac_malloc_chunk.h"
...
namespace util
{
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	void YAC_SpanAllocator::init(void *pAddr)
	{
		_pHead = static_cast<tagSpanAlloc*>(pAddr);
		_pData = (unsigned char*)((char*)_pHead + sizeof(tagSpanAlloc));
	}

	void YAC_SpanAllocator::create(void* pAddr, size_t iSpanSize, size_t iSpanCount)
	{
	...
	void YAC_SpanAllocator::connect(void *pAddr)
	{
		init(pAddr);
	}

	void* YAC_SpanAllocator::allocate()
	{
	...
	void YAC_SpanAllocator::deallocate(void *pAddr)
	{
		assert(pAddr >= _pData);
	...
	void YAC_SpanAllocator::rebuild()
	{
		assert(_pHead);
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	void YAC_Page::DLL_Init(YAC_Span* list, size_t iIndex)
	{
		list->next = iIndex;
		list->prev = iIndex;
	}
	...
	void YAC_Page::DLL_Remove(YAC_Span *span)
	{
		YAC_Span *_prev = reinterpret_cast<YAC_Span*>(reinterpret_cast<size_t>(_pShmFlagHead) + span->prev);
		YAC_Span *_next = reinterpret_cast<YAC_Span*>(reinterpret_cast<size_t>(_pShmFlagHead) + span->next);
	...
	void YAC_Page::DLL_Prepend(YAC_Span *list, YAC_Span *span)
	{
		size_t iBeginAddr = reinterpret_cast<size_t>(_pShmFlagHead);
		YAC_Span *_next = reinterpret_cast<YAC_Span*>(iBeginAddr + list->next);
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	YAC_Page::YAC_Span* YAC_Page::SearchFreeAndLargeLists(size_t n)
	{
		for (size_t s = n; s < kMaxPages; ++s)
		{
			YAC_Span* ll = &_pFree[s];
			if (!DLL_IsEmpty(ll, reinterpret_cast<size_t>(ll) - reinterpret_cast<size_t>(_pShmFlagHead)))
			{
				return Carve(reinterpret_cast<YAC_Span*>(reinterpret_cast<size_t>(_pShmFlagHead) + ll->next), n);
			}
		}
		return AllocLarge(n);
	}
	...
	YAC_Page::YAC_Span* YAC_Page::New(size_t n)
	{
		YAC_Span* result = SearchFreeAndLargeLists(n);
		if (result != NULL)
			return result;
	...
	YAC_Page::YAC_Span* YAC_Page::AllocLarge(size_t n)
	{
		YAC_Span *best = NULL;
		size_t iBeginAddr = reinterpret_cast<size_t>(_pShmFlagHead);

		YAC_Span* span = reinterpret_cast<YAC_Span*>(iBeginAddr + _pLarge->next);
		
		for (; span != _pLarge; span = reinterpret_cast<YAC_Span*>(iBeginAddr + span->next))
		{
			if (span->length >= n)
			{
	...
	YAC_Page::YAC_Span* YAC_Page::Carve(YAC_Span *span, size_t n)
	{
		...
		if (extra >= kMaxPages)
		{
			YAC_Span* leftover = static_cast<YAC_Span*>(_spanAlloc.allocate());
			if (flag)
			{
				update((void*)(&(leftover->start)), span->start);
				update((void*)(&(leftover->length)), n);
				update((void*)(&(leftover->location)), static_cast<unsigned int>(YAC_Span::ON_FREELIST));//update((void*)(&(leftover->location)), YAC_Span::IN_USE);
			...
			else
			{
				...
				leftover->length = n;
				leftover->location = YAC_Span::IN_USE;
				RecordSpan(leftover);
		...
		else
		{
			...
			if (flag)
			{
				...
				update((void*)(&(span->location)), static_cast<unsigned int>(YAC_Span::ON_FREELIST));
			}
			else
			{
				_prev->next = span->next;
			...
			if (extra > 0)
			{
				...
				YAC_Span* list = &_pFree[extra];

				size_t iBeginAddr = reinterpret_cast<size_t>(_pShmFlagHead);
				YAC_Span *_pnext = reinterpret_cast<YAC_Span*>(iBeginAddr + list->next);
	...
	void YAC_Page::PrependToFreeList(YAC_Span* span)
	{
		YAC_Span* list = (span->length < kMaxPages) ? &_pFree[span->length] : _pLarge;
		DLL_Prepend(list, span);
	}

	void YAC_Page::Delete(YAC_Span *span)
	{
		bool flag = _pShmFlagHead->_bShmProtectedArea;
		...
		if(flag)
		{
			...
			size_t location = YAC_Span::ON_FREELIST;
			size_t iBeginAddr = reinterpret_cast<size_t>(_pShmFlagHead);
			...
			if ( p > 0)
			{
				...
				if (pPrev != NULL && pPrev->location == location)
				{
					...
					else
					{
						...
						{
							YAC_Span *_prev = reinterpret_cast<YAC_Span*>(iBeginAddr + pPrev->prev);
							YAC_Span *_next = reinterpret_cast<YAC_Span*>(iBeginAddr + pPrev->next);
			...
			if ( p+n < _pShmFlagHead->_iShmPageMapNum)
			{
				...
				if (pNext != NULL && pNext->location == location)
				{
					...
					if (n >= kMaxPages && len >= kMaxPages)
					{
						if (prevMaxFlag)
						{
							YAC_Span *_prev = reinterpret_cast<YAC_Span*>(iBeginAddr + span->prev);
							YAC_Span *_next = reinterpret_cast<YAC_Span*>(iBeginAddr + span->next);
					...
					else if (n >= kMaxPages && len < kMaxPages)
					{
						if (prevMaxFlag)
						{
							...
							{
								YAC_Span *_prev = reinterpret_cast<YAC_Span*>(iBeginAddr + pNext->prev);
								YAC_Span *_next = reinterpret_cast<YAC_Span*>(iBeginAddr + pNext->next);
						...
						else
						{
							...
							YAC_Span* list = (n < kMaxPages) ? &_pFree[n] : _pLarge;		
		...
		{
			span->sizeclass = 0;
			span->location  = YAC_Span::ON_FREELIST;
			MergeIntoFreeList(span);
		}
	}

	void YAC_Page::MergeIntoFreeList(YAC_Span* span)
	{
		...
		if ( p > 0)
		{
			YAC_Span* pPrev = GetDescriptor(p-1);
		...
		if ( p+n < _pShmFlagHead->_iShmPageMapNum)
		{
			YAC_Span* pNext = GetDescriptor(p+n);
	...
	void YAC_Page::RegisterSizeClass(YAC_Span *span, size_t sc)
	{
		span->sizeclass = sc;
	...
	YAC_Page::YAC_Span* YAC_Page::GetDescriptor(size_t pPageId)
	{
		size_t end    = _pShmFlagHead->_iShmPageMapNum;

		if (pPageId >= 0 && pPageId < end)
		{
			return reinterpret_cast<YAC_Span*>(reinterpret_cast<char*>(_spanAlloc.getHead()) + Get(pPageId));
		}
		else
		{
			return NULL;
	...
	bool YAC_Page::UseShmMem()
	{
		...
		if (Ensure(p, size))
		{
			YAC_Span* span = static_cast<YAC_Span*>(_spanAlloc.allocate());
			...
			YAC_Span* list = (size < kMaxPages) ? &_pFree[size] : _pLarge;
	...
	void YAC_Page::RecordSpan(YAC_Span *span)
	{
		Set(span->start, reinterpret_cast<size_t>(span) - reinterpret_cast<size_t>(_spanAlloc.getHead()));
		if (span->length > 1)
		{
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	void* YAC_Page::fetchFromSpansSafe(size_t iClassSize, size_t &iAllocSize, size_t &iPageId, size_t &iIndex)
	{
	...
	void* YAC_Page::FetchFromSpans(size_t iClassSize, size_t &iAllocSize, size_t &iPageId, size_t &iIndex)
	{
		size_t iBeginAddr = reinterpret_cast<size_t>(_pShmFlagHead);
		if (DLL_IsEmpty(&_pCenterCache[iClassSize].nonempty, reinterpret_cast<size_t>(&_pCenterCache[iClassSize].nonempty) - iBeginAddr))
			return NULL;

		YAC_Span* span = reinterpret_cast<YAC_Span*>(iBeginAddr + _pCenterCache[iClassSize].nonempty.next);
		...
		if (p == 0)
		{
			...
			YAC_Span *list  = &(_pCenterCache[iClassSize].empty);
			_next = reinterpret_cast<YAC_Span*>(iBeginAddr + list->next);
			if (flag)
			{
				update((void*)(&(span->next)), list->next);
				update((void*)(&(span->prev)), reinterpret_cast<size_t>(list) - iBeginAddr);
	...
	int YAC_Page::Populate(size_t iClassSize)
	{
		const size_t npages = Static::sizemap()->class_to_pages(iClassSize);
		bool      flag		= _pShmFlagHead->_bShmProtectedArea;

		YAC_Span* span = New(npages);

		if (span)
		{
			if (flag)
			{
				update((void*)(&(span->location)), static_cast<unsigned int>(YAC_Span::IN_USE));
		...
		*tail = reinterpret_cast<size_t>(_ptr) - iBeginAddr;
		...
		YAC_Span *list = &(_pCenterCache[iClassSize].nonempty);
		YAC_Span *_next = reinterpret_cast<YAC_Span*>(iBeginAddr + list->next);
		if (flag)
		{
			update((void*)(&(span->next)), list->next);
	...
	void YAC_Page::releaseToSpans(size_t iPageId, size_t iIndex)
	{
		YAC_Span* span = GetDescriptor(iPageId);
		assert(span != NULL);
		...
		if (spanObject == 0)
		{
			...
			YAC_Span *list  = &(_pCenterCache[_size_class].nonempty);
			_next = reinterpret_cast<YAC_Span*>(iBeginAddr + list->next);
			if (flag)
			{
				update((void*)(&(span->next)), list->next);
	...
	void* YAC_Page::getAbsolute(size_t iPageId, size_t iIndex)
	{
		YAC_Span* span = GetDescriptor(iPageId);
		assert(span != NULL);
		const size_t _size_class = span->sizeclass;
	...
	void YAC_Page::doUpdate(bool bUpdate)
	{
		if(bUpdate)
		{
	...
	void YAC_Page::init(void *pAddr)
	{
		...
		_pCenterCache   = static_cast<YAC_CenterList*>((void*)((char*)_pModifyHead + sizeof(tagModifyHead)));

		_pLarge         = static_cast<YAC_Span*>((void*)((char*)_pCenterCache + sizeof(YAC_CenterList) * kNumClasses));

		_pFree          = static_cast<YAC_Span*>((void*)((char*)_pLarge + sizeof(YAC_Span)));
	}

	void YAC_Page::initPage(void *pAddr)
	{
		void * span_begin_ptr				= static_cast<void*>((char*)_pFree + sizeof(YAC_Span) * kMaxPages);
		size_t begin						= reinterpret_cast<size_t>(pAddr);
		size_t end							= reinterpret_cast<size_t>(span_begin_ptr);

		_pSpanMemHead						= static_cast<YAC_SpanAllocator*>((void*)((char*)span_begin_ptr));
		_pShmFlagHead->_iShmSpanAddr		= end - begin;

		size_t _iCapacitySize				= _pShmFlagHead->_iShmMemSize - (end - begin);
		size_t _iPagesNum					= (_iCapacitySize - YAC_SpanAllocator::getHeadSize() - sizeof(YAC_Span) * 20) / (sizeof(YAC_Span) + sizeof(size_t) + kPageSize);

		size_t*    pagemap_ptr				= reinterpret_cast<size_t*>((char*)_pSpanMemHead + YAC_SpanAllocator::getHeadSize() + sizeof(YAC_Span) * (_iPagesNum + 20));
		...
		_spanAlloc.create((void*)((char*)_pSpanMemHead), sizeof(YAC_Span), (_iPagesNum + 20));
	...
	void YAC_Page::create(void *pAddr, size_t iShmMemSize, bool bProtectedArea)
	{
		init(pAddr);
	...
	void YAC_Page::connect(void *pAddr)
	{
		init(pAddr);
		size_t iBeginAddr = reinterpret_cast<size_t>(pAddr);

		_pSpanMemHead	= reinterpret_cast<YAC_SpanAllocator*>(iBeginAddr + _pShmFlagHead->_iShmSpanAddr);
	...
	size_t YAC_Page::getPageNumber()
	{
		return _pShmFlagHead->_iShmPageNum;
	}

	size_t YAC_Page::getPageMemSize()
	{
		return _pShmFlagHead->_iShmPageNum << kPageShift;
	}

	size_t YAC_Page::getPageMemEnd()
	{
		return reinterpret_cast<size_t>(_pShmFlagHead) + _pShmFlagHead->_iShmMemSize;
	}

	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	void YAC_MallocChunkAllocator::init(void *pAddr)
	{
		_pHead	= static_cast<tagChunkAllocatorHead*>(pAddr);
		_pChunk = (char*)_pHead + sizeof(tagChunkAllocatorHead);
	}

	void YAC_MallocChunkAllocator::create(void *pAddr, size_t iSize, bool bProtectedArea)
	{
		assert(iSize > (sizeof(tagChunkAllocatorHead) + YAC_Page::getMinMemSize()));
		init(pAddr);
	...
	void YAC_MallocChunkAllocator::_connect(void *pAddr)
	{
		clear();
		...
		_nallocator = new YAC_MallocChunkAllocator();
		_nallocator->connect(pNextHead);
	}

	void YAC_MallocChunkAllocator::connect(void *pAddr)
	{
		_connect(pAddr);
		doUpdate();
	...
	void YAC_MallocChunkAllocator::rebuild()
	{
		_page.rebuild();
	...
	YAC_MallocChunkAllocator * YAC_MallocChunkAllocator::lastAlloc()
	{
		if(_nallocator == NULL)
			return NULL;

		YAC_MallocChunkAllocator *p = _nallocator;
	...
	void YAC_MallocChunkAllocator::append(void *pAddr, size_t iSize)
	{
		connect(pAddr);
		...
		YAC_MallocChunkAllocator *p = new YAC_MallocChunkAllocator();
		p->create(pAppendAddr, iSize - _pHead->_iTotalSize, _pHead->_bProtectedArea);

		YAC_MallocChunkAllocator *palloc = lastAlloc();
		if (palloc)
		{
	...
	void* YAC_MallocChunkAllocator::allocate(size_t iNeedSize, size_t &iAllocSize)
	{
		size_t iPageId = 0;
		size_t iIndex   = 0;
	...
	void  YAC_MallocChunkAllocator::deallocate(void* pAddr)
	{
		if (_nallocator)
		{
	...
	void* YAC_MallocChunkAllocator::allocate(size_t iNeedSize, size_t &iAllocSize, size_t &iPageId, size_t &iIndex)
	{
	...
	void YAC_MallocChunkAllocator::deallocate(size_t iPageId, size_t iIndex)
	{
		if (_nallocator)
		{
			size_t prev = _page.getPageNumber();
	...
	void* YAC_MallocChunkAllocator::getAbsolute(size_t iPageId, size_t iIndex)
	{
		if(_nallocator == NULL)
		{
	...
	size_t YAC_MallocChunkAllocator::getAllCapacity()
	{	
	...
	void   YAC_MallocChunkAllocator::getSingleCapacity(vector<size_t> &vec_capacity)
	{
		vec_capacity.clear();
	...
	void YAC_MallocChunkAllocator::doUpdate(bool bUpdate)
	{
