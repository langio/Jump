...
int YAC_HashMap::Block::getBlockData(YAC_HashMap::BlockData &data)
{
	data._dirty = isDirty();
    ...
    try
    {
        ...
        {
            return YAC_HashMap::RT_ONLY_KEY;
        }
    }
    catch(exception &ex)
    {
        ret = YAC_HashMap::RT_DECODE_ERR;
    }

    return ret;
}

size_t YAC_HashMap::Block::getLastBlockHead()
{
	size_t iHead = _iHead;
...
int YAC_HashMap::Block::get(void *pData, size_t &iDataLen)
{
	...
	else
	{
		...
		{
			return YAC_HashMap::RT_NOTALL_ERR;   //copy数据不完全
		}
		...
		while(iHasLen < iDataLen)
		{
			...
			if(!pChunk->_bNextChunk)
			{
				...
				{
					return YAC_HashMap::RT_NOTALL_ERR;       //copy不完全
				}
...
int YAC_HashMap::Block::get(string &s)
{
...
int YAC_HashMap::Block::set(const void *pData, size_t iDataLen, bool bOnlyKey, vector<YAC_HashMap::BlockData> &vtData)
{
...
void YAC_HashMap::Block::setDirty(bool b)
{
...
bool YAC_HashMap::Block::nextBlock()
{
...
bool YAC_HashMap::Block::prevBlock()
{
...
void YAC_HashMap::Block::deallocate()
{
	vector<size_t> v;
	v.push_back(_iHead);
...
void YAC_HashMap::Block::makeNew(size_t index, size_t iAllocSize)
{
...
void YAC_HashMap::Block::erase()
{
	...
	{
		//如果是hash桶的头部, 则还需要处理
		YAC_HashMap::tagHashItem *pItem  = _pMap->item(getBlockHead()->_iIndex);
...
void YAC_HashMap::Block::refreshSetList()
{
...
void YAC_HashMap::Block::refreshGetList()
{
...
void YAC_HashMap::Block::deallocate(size_t iChunk)
{
...
int YAC_HashMap::Block::allocate(size_t iDataLen, vector<YAC_HashMap::BlockData> &vtData)
{
	size_t fn   = 0;
...
int YAC_HashMap::Block::joinChunk(tagChunkHead *pChunk, const vector<size_t> chunks)
{
...
int YAC_HashMap::Block::allocateChunk(size_t fn, vector<size_t> &chunks, vector<YAC_HashMap::BlockData> &vtData)
{
    assert(fn > 0);
	...
	while(true)
	{
		...
		if (t == 0)
		{
			...
			chunks.clear();
			return YAC_HashMap::RT_NO_MEMORY;
		}
...
size_t YAC_HashMap::Block::getDataLen()
{
	size_t n = 0;
	if(!getBlockHead()->_bNextChunk)
	{
...
size_t YAC_HashMap::BlockAllocator::allocateMemBlock(size_t index, size_t &iAllocSize, vector<YAC_HashMap::BlockData> &vtData)
{
begin:
...
size_t YAC_HashMap::BlockAllocator::allocateChunk(size_t iAddr, size_t &iAllocSize, vector<YAC_HashMap::BlockData> &vtData)
{
begin:
	void *pAddr = _pChunkAllocator->allocate(iAllocSize, iAllocSize);
...
void YAC_HashMap::BlockAllocator::deallocateMemBlock(const vector<size_t> &v)
{
...
void YAC_HashMap::BlockAllocator::deallocateMemBlock(size_t v)
{
...
YAC_HashMap::HashMapLockItem::HashMapLockItem(YAC_HashMap *pMap, size_t iAddr)
: _pMap(pMap)
, _iAddr(iAddr)
{
}

YAC_HashMap::HashMapLockItem::HashMapLockItem(const HashMapLockItem &mcmdi)
: _pMap(mcmdi._pMap)
, _iAddr(mcmdi._iAddr)
{
}

YAC_HashMap::HashMapLockItem &YAC_HashMap::HashMapLockItem::operator=(const YAC_HashMap::HashMapLockItem &mcmdi)
{
    if(this != &mcmdi)
    {
        _pMap   = mcmdi._pMap;
...
bool YAC_HashMap::HashMapLockItem::operator==(const YAC_HashMap::HashMapLockItem &mcmdi)
{
...
bool YAC_HashMap::HashMapLockItem::isDirty()
{
    Block block(_pMap, _iAddr);
    return block.isDirty();
}

bool YAC_HashMap::HashMapLockItem::isOnlyKey()
{
    Block block(_pMap, _iAddr);
    return block.isOnlyKey();
}

time_t YAC_HashMap::HashMapLockItem::getSyncTime()
{
    Block block(_pMap, _iAddr);
    return block.getSyncTime();
}

int YAC_HashMap::HashMapLockItem::get(string& k, string& v)
{
	string s;
	...
	{
		return YAC_HashMap::RT_EXCEPTION_ERR;
	}

	return YAC_HashMap::RT_OK;
}

int YAC_HashMap::HashMapLockItem::get(string& k)
{
	string s;
...
int YAC_HashMap::HashMapLockItem::set(const string& k, const string& v, vector<YAC_HashMap::BlockData> &vtData)
{
	Block block(_pMap, _iAddr);
...
int YAC_HashMap::HashMapLockItem::set(const string& k, vector<YAC_HashMap::BlockData> &vtData)
{
	Block block(_pMap, _iAddr);
...
bool YAC_HashMap::HashMapLockItem::equal(const string &k, string &v, int &ret)
{
	string k1;
	ret = get(k1, v);

	if ((ret == YAC_HashMap::RT_OK || ret == YAC_HashMap::RT_ONLY_KEY) && k == k1)
	{
		return true;
	}
...
bool YAC_HashMap::HashMapLockItem::equal(const string& k, int &ret)
{
	string k1;
	ret = get(k1);

	if (ret == YAC_HashMap::RT_OK && k == k1)
	{
		return true;
	}
...
void YAC_HashMap::HashMapLockItem::nextItem(int iType)
{
	Block block(_pMap, _iAddr);
...
void YAC_HashMap::HashMapLockItem::prevItem(int iType)
{
	Block block(_pMap, _iAddr);
...
YAC_HashMap::HashMapLockIterator::HashMapLockIterator()
: _pMap(NULL), _iItem(NULL, 0)
{
}

YAC_HashMap::HashMapLockIterator::HashMapLockIterator(YAC_HashMap *pMap, size_t iAddr, int iType, int iOrder)
: _pMap(pMap), _iItem(_pMap, iAddr), _iType(iType), _iOrder(iOrder)
{
}

YAC_HashMap::HashMapLockIterator::HashMapLockIterator(const HashMapLockIterator &it)
: _pMap(it._pMap),_iItem(it._iItem), _iType(it._iType), _iOrder(it._iOrder)
{
}

YAC_HashMap::HashMapLockIterator& YAC_HashMap::HashMapLockIterator::operator=(const HashMapLockIterator &it)
{
    if(this != &it)
    {
        _pMap       = it._pMap;
...
bool YAC_HashMap::HashMapLockIterator::operator==(const HashMapLockIterator& mcmi)
{
...
YAC_HashMap::HashMapLockIterator& YAC_HashMap::HashMapLockIterator::operator++()
{
	if(_iOrder == IT_NEXT)
	{
...
YAC_HashMap::HashMapLockIterator YAC_HashMap::HashMapLockIterator::operator++(int)
{
...
YAC_HashMap::HashMapItem::HashMapItem(YAC_HashMap *pMap, size_t iIndex)
: _pMap(pMap)
, _iIndex(iIndex)
{
}

YAC_HashMap::HashMapItem::HashMapItem(const HashMapItem &mcmdi)
: _pMap(mcmdi._pMap)
, _iIndex(mcmdi._iIndex)
{
}

YAC_HashMap::HashMapItem &YAC_HashMap::HashMapItem::operator=(const YAC_HashMap::HashMapItem &mcmdi)
{
    if(this != &mcmdi)
    {
        _pMap   = mcmdi._pMap;
...
bool YAC_HashMap::HashMapItem::operator==(const YAC_HashMap::HashMapItem &mcmdi)
{
...
void YAC_HashMap::HashMapItem::get(vector<YAC_HashMap::BlockData> &vtData)
{
...
void YAC_HashMap::HashMapItem::nextItem()
{
    if(_iIndex == (size_t)(-1))
    {
        return;
...
int YAC_HashMap::HashMapItem::setDirty()
{
...
YAC_HashMap::HashMapIterator::HashMapIterator()
: _pMap(NULL), _iItem(NULL, 0)
{
}

YAC_HashMap::HashMapIterator::HashMapIterator(YAC_HashMap *pMap, size_t iIndex)
: _pMap(pMap), _iItem(_pMap, iIndex)
{
}

YAC_HashMap::HashMapIterator::HashMapIterator(const HashMapIterator &it)
: _pMap(it._pMap),_iItem(it._iItem)
{
}

YAC_HashMap::HashMapIterator& YAC_HashMap::HashMapIterator::operator=(const HashMapIterator &it)
{
    if(this != &it)
    {
        _pMap       = it._pMap;
...
bool YAC_HashMap::HashMapIterator::operator==(const HashMapIterator& mcmi)
{
...
YAC_HashMap::HashMapIterator& YAC_HashMap::HashMapIterator::operator++()
{
	_iItem.nextItem();
	return (*this);
}

YAC_HashMap::HashMapIterator YAC_HashMap::HashMapIterator::operator++(int)
{
...
void YAC_HashMap::init(void *pAddr)
{
	_pHead          = static_cast<tagMapHead*>(pAddr);
	_pstModifyHead  = static_cast<tagModifyHead*>((void*)((char*)pAddr + sizeof(tagMapHead)));
}

void YAC_HashMap::initDataBlockSize(size_t iMinDataSize, size_t iMaxDataSize, float fFactor)
{
...
void YAC_HashMap::create(void *pAddr, size_t iSize)
{
	if(sizeof(tagHashItem) * 1
	   ...
	   + 10 > iSize)
	{
        throw YAC_HashMap_Exception("[YAC_HashMap::create] mem size not enougth.");
    }

    if(_iMinDataSize == 0 || _iMaxDataSize == 0 || _fFactor < 1.0)
    {
        throw YAC_HashMap_Exception("[YAC_HashMap::create] init data size error:" + YAC_Common::tostr(_iMinDataSize) + "|" + YAC_Common::tostr(_iMaxDataSize) + "|" + YAC_Common::tostr(_fFactor));
    }
	...
	//Hash个数
	size_t iHashCount   = (iSize - sizeof(YAC_MemChunkAllocator::tagChunkAllocatorHead)) / ((size_t)(iBlockSize*_fRadio) + sizeof(tagHashItem));
    //采用最近的素数作为hash值
    iHashCount          = getMinPrimeNumber(iHashCount);

	void *pHashAddr     = (char*)_pHead + sizeof(tagMapHead) + sizeof(tagModifyHead);

    size_t iHashMemSize = YAC_MemVector<tagHashItem>::calcMemSize(iHashCount);
...
void YAC_HashMap::connect(void *pAddr, size_t iSize)
{
	init(pAddr);
	...
	if(_pHead->_cMaxVersion != MAX_VERSION || _pHead->_cMinVersion != MIN_VERSION)
	{
        ...
        throw YAC_HashMap_Exception("[YAC_HashMap::connect] hash map version not equal:" + os.str() + " (data != code)");
	}

    if(_pHead->_iMemSize != iSize)
    {
        throw YAC_HashMap_Exception("[YAC_HashMap::connect] hash map size not equal:" + YAC_Common::tostr(_pHead->_iMemSize) + "!=" + YAC_Common::tostr(iSize));
    }
...
int YAC_HashMap::append(void *pAddr, size_t iSize)
{
    ...
    if(_pHead->_cMaxVersion != MAX_VERSION || _pHead->_cMinVersion != MIN_VERSION)
    {
        ostringstream os;
        os << (int)_pHead->_cMaxVersion << "." << (int)_pHead->_cMinVersion << " != " << ((int)MAX_VERSION) << "." << ((int)MIN_VERSION);
        throw YAC_HashMap_Exception("[YAC_HashMap::append] hash map version not equal:" + os.str() + " (data != code)");
    }
...
int YAC_HashMap::dump2file(const string &sFile)
{
...
int YAC_HashMap::load5file(const string &sFile)
{
...
int YAC_HashMap::recover(size_t i, bool bRepair)
{
    doUpdate();
...
check:
    ...
    while(block.getHead() != 0)
    {
        BlockData data;
        int ret = block.getBlockData(data);
        if(ret != YAC_HashMap::RT_OK && ret != YAC_HashMap::RT_ONLY_KEY)
        {
            //增加删除block数
            ++e;
...
int YAC_HashMap::checkDirty(const string &k)
{
	doUpdate();
	incGetCount();

	int ret         = YAC_HashMap::RT_OK;
	...
	if (block.isDirty())
	{
		return YAC_HashMap::RT_DIRTY_DATA;
	}

	return YAC_HashMap::RT_OK;
}

int YAC_HashMap::setDirty(const string& k)
{
	doUpdate();
...
int YAC_HashMap::setDirtyAfterSync(const string& k)
{
	doUpdate();
...
int YAC_HashMap::setClean(const string& k)
{
	doUpdate();
...
int YAC_HashMap::get(const string& k, string &v, time_t &iSyncTime)
{
	doUpdate();
	incGetCount();
...
int YAC_HashMap::get(const string& k, string &v)
{
    time_t iSyncTime;
    return get(k, v, iSyncTime);
}

int YAC_HashMap::set(const string& k, const string& v, bool bDirty, vector<BlockData> &vtData)
{
	doUpdate();
	incGetCount();
...
int YAC_HashMap::set(const string& k, vector<BlockData> &vtData)
{
	doUpdate();
	incGetCount();
...
int YAC_HashMap::del(const string& k, BlockData &data)
{
	doUpdate();
	incGetCount();
...
int YAC_HashMap::erase(int radio, BlockData &data, bool bCheckDirty)
{
    doUpdate();
    ...
    if(ret == YAC_HashMap::RT_OK)
    {
        return YAC_HashMap::RT_ERASE_OK;
    }
    return ret;
}

void YAC_HashMap::sync()
{
    doUpdate();
...
int YAC_HashMap::sync(time_t iNowTime, BlockData &data)
{
    doUpdate();
...
void YAC_HashMap::backup(bool bForceFromBegin)
{
    doUpdate();
...
int YAC_HashMap::backup(BlockData &data)
{
    doUpdate();
    ...
    if(ret == YAC_HashMap::RT_OK)
    {
        return YAC_HashMap::RT_NEED_BACKUP;
    }
    return ret;
}

YAC_HashMap::lock_iterator YAC_HashMap::find(const string& k)
{
	size_t index = hashIndex(k);
	int ret = YAC_HashMap::RT_OK;
...
YAC_HashMap::lock_iterator YAC_HashMap::begin()
{
	doUpdate();
...
YAC_HashMap::lock_iterator YAC_HashMap::rbegin()
{
	doUpdate();
...
size_t YAC_HashMap::eraseExcept(size_t iNowAddr, vector<BlockData> &vtData)
{
    //不能被淘汰
	...
	while (d != 0)
	{
        ...
        //判断按照哪种方式淘汰
        if(_pHead->_cEraseMode == YAC_HashMap::ERASEBYSET)
        {
        ...
        else if(ret == YAC_HashMap::RT_NO_DATA)
        {
            d--;
        }
...
size_t YAC_HashMap::hashIndex(const string& k)
{
	return _hashf(k) % _hash.size();
}

YAC_HashMap::lock_iterator YAC_HashMap::find(const string& k, size_t index, string &v, int &ret)
{
	ret = YAC_HashMap::RT_OK;
...
YAC_HashMap::lock_iterator YAC_HashMap::find(const string& k, size_t index, int &ret)
{
	ret = YAC_HashMap::RT_OK;
...
void YAC_HashMap::analyseHash(uint32_t &iMaxHash, uint32_t &iMinHash, float &fAvgHash)
{
	iMaxHash = 0;
	iMinHash = (uint32_t)-1;
...
void YAC_HashMap::doUpdate(bool bUpdate)
{
	if(bUpdate)
	{
...
void YAC_HashMap::update(void* iModifyAddr, size_t iModifyValue)
{
...
#if __WORDSIZE == 64
void YAC_HashMap::update(void* iModifyAddr, uint32_t iModifyValue)
{
	_pstModifyHead->_cModifyStatus = 1;
	_pstModifyHead->_stModifyData[_pstModifyHead->_iNowIndex]._iModifyAddr  = getRelative(iModifyAddr);
	_pstModifyHead->_stModifyData[_pstModifyHead->_iNowIndex]._iModifyValue = iModifyValue;
...
void YAC_HashMap::update(void* iModifyAddr, bool bModifyValue)
{
	_pstModifyHead->_cModifyStatus = 1;
	_pstModifyHead->_stModifyData[_pstModifyHead->_iNowIndex]._iModifyAddr  = getRelative(iModifyAddr);
...
size_t YAC_HashMap::getMinPrimeNumber(size_t n)
{
    while(true)
    {
