#include "util/yac_multi_hashmap.h"
#include "util/yac_pack.h"
#include "util/yac_common.h"

namespace util
{

int YAC_Multi_HashMap::FailureRecover::_iRefCount = 0;

bool YAC_Multi_HashMap::MainKey::next()
{
...
bool YAC_Multi_HashMap::MainKey::prev()
{
...
void YAC_Multi_HashMap::MainKey::deallocate()
{
	if(HASNEXTCHUNK())
	{
		// 释放所有chunk
...
void YAC_Multi_HashMap::MainKey::deallocate(uint32_t iChunk)
{
	vector<uint32_t> v;
	v.push_back(iChunk);
...
void YAC_Multi_HashMap::MainKey::makeNew(uint32_t iIndex, uint32_t iAllocSize)
{
...
int YAC_Multi_HashMap::MainKey::erase(vector<Value> &vtData)
{
    //////////////////修改备份数据链表/////////////
	...
	string mk;
	int ret = get(mk);
	if(ret != YAC_Multi_HashMap::RT_OK)
	{
	...
	{
		//如果是hash桶的头部, 则还需要处理
		YAC_Multi_HashMap::tagMainKeyHashItem *pItem  = _pMap->itemMainKey(getHeadPtr()->_iIndex);
...
void YAC_Multi_HashMap::MainKey::refreshGetList()
{
...
int YAC_Multi_HashMap::MainKey::allocate(uint32_t iDataLen, vector<YAC_Multi_HashMap::Value> &vtData)
{
	uint32_t fn   = 0;
    //一个块的真正的数据容量
	...
	if(fn >= iDataLen)
	{
		...
		return YAC_Multi_HashMap::RT_OK;
	}
...
int YAC_Multi_HashMap::MainKey::joinChunk(tagChunkHead *pChunk, const vector<uint32_t>& chunks)
{
...
int YAC_Multi_HashMap::MainKey::allocateChunk(uint32_t fn, vector<uint32_t> &chunks, vector<YAC_Multi_HashMap::Value> &vtData)
{
    assert(fn > 0);
	...
	while(true)
	{
		...
		if (t == 0)
		{
			...
			chunks.clear();
			return YAC_Multi_HashMap::RT_NO_MEMORY;
		}
...
uint32_t YAC_Multi_HashMap::MainKey::getDataLen()
{
	uint32_t n = 0;
	if(!HASNEXTCHUNK())
	{
...
int YAC_Multi_HashMap::MainKey::get(void *pData, uint32_t &iDataLen)
{
	if(!HASNEXTCHUNK())
	{
	...
	else
	{
		...
		{
			return YAC_Multi_HashMap::RT_NOTALL_ERR;   // copy数据不完全
		}
		...
		while(iHasLen < iDataLen)
		{
			...
			if(!pChunk->_bNextChunk)
			{
				...
				{
					return YAC_Multi_HashMap::RT_NOTALL_ERR;       // copy不完全
				}
...
int YAC_Multi_HashMap::MainKey::get(string &mk)
{
	// 获取数据长度
...
int YAC_Multi_HashMap::MainKey::set(const void *pData, uint32_t iDataLen, vector<YAC_Multi_HashMap::Value> &vtData)
{
...
int YAC_Multi_HashMap::Block::getBlockData(YAC_Multi_HashMap::BlockData &data)
{
	data._dirty = isDirty();
    ...
    try
    {
        ...
        YAC_PackOut po(s.c_str(), s.length());
        po >> data._key;
        ...
        {
            return YAC_Multi_HashMap::RT_ONLY_KEY;
        }
    }
    catch(exception &ex)
    {
        ret = YAC_Multi_HashMap::RT_DECODE_ERR;
    }

    return ret;
}

uint32_t YAC_Multi_HashMap::Block::getLastBlockHead(bool bUKList)
{
...
int YAC_Multi_HashMap::Block::get(void *pData, uint32_t &iDataLen)
{
	if(!HASNEXTCHUNK())
	{
		// 没有下一个chunk, 一个chunk就可以装下数据了
...
int YAC_Multi_HashMap::Block::get(string &s)
{
	uint32_t iLen = getDataLen();
...
int YAC_Multi_HashMap::Block::set(const void *pData, uint32_t iDataLen, bool bOnlyKey, 
								 uint8_t iVersion, vector<YAC_Multi_HashMap::Value> &vtData)
{
	// version为0表示外部不关心数据版本，onlykey时也不需要关注数据版本
	if(iVersion != 0 && !isOnlyKey() && getBlockHead()->_iVersion != iVersion)
	{
		// 数据版本不匹配
		return YAC_Multi_HashMap::RT_DATA_VER_MISMATCH;
	}
...
void YAC_Multi_HashMap::Block::setDirty(bool b)
{
	if (b)
	{
		if(!isDirty())
...
bool YAC_Multi_HashMap::Block::nextBlock()
{
...
bool YAC_Multi_HashMap::Block::prevBlock()
{
...
void YAC_Multi_HashMap::Block::deallocate()
{
	// 先释放所有的后续chunk
	if(HASNEXTCHUNK())
	{
...
void YAC_Multi_HashMap::Block::makeNew(uint32_t iMainKeyAddr, uint32_t uIndex, uint32_t iAllocSize, bool bHead)
{
...
void YAC_Multi_HashMap::Block::erase()
{
	//////////////////修改脏数据链表/////////////
	...
	{
		//如果是hash桶的头部, 则还需要处理
		YAC_Multi_HashMap::tagHashItem *pItem  = _pMap->item(getBlockHead()->_iIndex);
...
void YAC_Multi_HashMap::Block::refreshSetList()
{
...
void YAC_Multi_HashMap::Block::deallocate(uint32_t iChunk)
{
	tagChunkHead *pChunk = getChunkHead(iChunk);
	vector<uint32_t> v;
	v.push_back(iChunk);
...
int YAC_Multi_HashMap::Block::allocate(uint32_t iDataLen, vector<YAC_Multi_HashMap::Value> &vtData)
{
	uint32_t fn   = 0;
...
int YAC_Multi_HashMap::Block::joinChunk(tagChunkHead *pChunk, const vector<uint32_t>& chunks)
{
	for (size_t i = 0; i < chunks.size(); ++i)
	{
		if (i == chunks.size() - 1)
...
int YAC_Multi_HashMap::Block::allocateChunk(uint32_t fn, vector<uint32_t> &chunks, vector<YAC_Multi_HashMap::Value> &vtData)
{
    assert(fn > 0);
...
uint32_t YAC_Multi_HashMap::Block::getDataLen()
{
	uint32_t n = 0;
	if(!HASNEXTCHUNK())
	{
...
uint32_t YAC_Multi_HashMap::BlockAllocator::allocateMemBlock(uint32_t iMainKeyAddr, uint32_t index, bool bHead, 
														  uint32_t &iAllocSize, vector<YAC_Multi_HashMap::Value> &vtData)
{
begin:
	size_t iAddr = 0;
...
uint32_t YAC_Multi_HashMap::BlockAllocator::allocateMainKeyHead(uint32_t index, vector<YAC_Multi_HashMap::Value> &vtData)
{
...
uint32_t YAC_Multi_HashMap::BlockAllocator::allocateChunk(uint32_t iAddr, uint32_t &iAllocSize, vector<YAC_Multi_HashMap::Value> &vtData)
{
begin:
	size_t iChunkAddr = 0;
	size_t bigSize = iAllocSize;
...
void YAC_Multi_HashMap::BlockAllocator::deallocateMemChunk(const vector<uint32_t> &v)
{
...
void YAC_Multi_HashMap::BlockAllocator::deallocateMemChunk(uint32_t iChunk)
{
...
YAC_Multi_HashMap::HashMapLockItem::HashMapLockItem(YAC_Multi_HashMap *pMap, uint32_t iAddr)
: _pMap(pMap)
, _iAddr(iAddr)
{
}

YAC_Multi_HashMap::HashMapLockItem::HashMapLockItem(const HashMapLockItem &mcmdi)
: _pMap(mcmdi._pMap)
, _iAddr(mcmdi._iAddr)
{
}

YAC_Multi_HashMap::HashMapLockItem &YAC_Multi_HashMap::HashMapLockItem::operator=(const YAC_Multi_HashMap::HashMapLockItem &mcmdi)
{
    if(this != &mcmdi)
    {
        _pMap   = mcmdi._pMap;
...
bool YAC_Multi_HashMap::HashMapLockItem::operator==(const YAC_Multi_HashMap::HashMapLockItem &mcmdi)
{
...
bool YAC_Multi_HashMap::HashMapLockItem::isDirty()
{
    Block block(_pMap, _iAddr);
    return block.isDirty();
}

bool YAC_Multi_HashMap::HashMapLockItem::isOnlyKey()
{
    Block block(_pMap, _iAddr);
    return block.isOnlyKey();
}

time_t YAC_Multi_HashMap::HashMapLockItem::getSyncTime()
{
    Block block(_pMap, _iAddr);
    return block.getSyncTime();
}

int YAC_Multi_HashMap::HashMapLockItem::get(YAC_Multi_HashMap::Value &v)
{
	Block block(_pMap, _iAddr);
...
int YAC_Multi_HashMap::HashMapLockItem::get(string &mk, string &uk)
{
	Block block(_pMap, _iAddr);
	MainKey mainKey(_pMap, block.getBlockHead()->_iMainKey);
	...
	{
		return YAC_Multi_HashMap::RT_EXCEPTION_ERR;
	}

	return YAC_Multi_HashMap::RT_OK;
}

int YAC_Multi_HashMap::HashMapLockItem::set(const string &mk, const string &uk, 
										   const string &v, uint8_t iVersion, vector<YAC_Multi_HashMap::Value> &vtData)
{
	Block block(_pMap, _iAddr);
...
int YAC_Multi_HashMap::HashMapLockItem::set(const string &mk, const string &uk, vector<YAC_Multi_HashMap::Value> &vtData)
{
	Block block(_pMap, _iAddr);
...
bool YAC_Multi_HashMap::HashMapLockItem::equal(const string &mk, const string &uk, YAC_Multi_HashMap::Value &v, int &ret)
{
	ret = get(v);

	if ((ret == YAC_Multi_HashMap::RT_OK || ret == YAC_Multi_HashMap::RT_ONLY_KEY) 
...
bool YAC_Multi_HashMap::HashMapLockItem::equal(const string &mk, const string &uk, int &ret)
{
	string mk1, uk1;
	ret = get(mk1, uk1);

	if (ret == YAC_Multi_HashMap::RT_OK && mk == mk1 && uk == uk1)
	{
		return true;
	}
...
void YAC_Multi_HashMap::HashMapLockItem::nextItem(int iType)
{
	if(_iAddr == 0)
	{
		// 到头了
...
void YAC_Multi_HashMap::HashMapLockItem::prevItem(int iType)
{
	if(_iAddr == 0)
	{
		// 到头了
...
YAC_Multi_HashMap::HashMapLockIterator::HashMapLockIterator()
: _pMap(NULL), _iItem(NULL, 0)
{
}

YAC_Multi_HashMap::HashMapLockIterator::HashMapLockIterator(YAC_Multi_HashMap *pMap, uint32_t iAddr, int iType, int iOrder)
: _pMap(pMap), _iItem(_pMap, iAddr), _iType(iType), _iOrder(iOrder)
{
}

YAC_Multi_HashMap::HashMapLockIterator::HashMapLockIterator(const HashMapLockIterator &it)
: _pMap(it._pMap),_iItem(it._iItem), _iType(it._iType), _iOrder(it._iOrder)
{
}

YAC_Multi_HashMap::HashMapLockIterator& YAC_Multi_HashMap::HashMapLockIterator::operator=(const HashMapLockIterator &it)
{
    if(this != &it)
    {
        _pMap       = it._pMap;
...
bool YAC_Multi_HashMap::HashMapLockIterator::operator==(const HashMapLockIterator& mcmi)
{
...
YAC_Multi_HashMap::HashMapLockIterator& YAC_Multi_HashMap::HashMapLockIterator::operator++()
{
	if(_iOrder == IT_NEXT)
	{
		// 顺序遍历
...
YAC_Multi_HashMap::HashMapLockIterator YAC_Multi_HashMap::HashMapLockIterator::operator++(int)
{
...
YAC_Multi_HashMap::HashMapItem::HashMapItem(YAC_Multi_HashMap *pMap, uint32_t iIndex)
: _pMap(pMap)
, _iIndex(iIndex)
{
}

YAC_Multi_HashMap::HashMapItem::HashMapItem(const HashMapItem &mcmdi)
: _pMap(mcmdi._pMap)
, _iIndex(mcmdi._iIndex)
{
}

YAC_Multi_HashMap::HashMapItem &YAC_Multi_HashMap::HashMapItem::operator=(const YAC_Multi_HashMap::HashMapItem &mcmdi)
{
    if(this != &mcmdi)
    {
        _pMap   = mcmdi._pMap;
...
bool YAC_Multi_HashMap::HashMapItem::operator==(const YAC_Multi_HashMap::HashMapItem &mcmdi)
{
...
void YAC_Multi_HashMap::HashMapItem::get(vector<YAC_Multi_HashMap::Value> &vtData)
{
...
void YAC_Multi_HashMap::HashMapItem::nextItem()
{
...
YAC_Multi_HashMap::HashMapIterator::HashMapIterator()
: _pMap(NULL), _iItem(NULL, (uint32_t)-1)
{
}

YAC_Multi_HashMap::HashMapIterator::HashMapIterator(YAC_Multi_HashMap *pMap, uint32_t iIndex)
: _pMap(pMap), _iItem(_pMap, iIndex)
{
}

YAC_Multi_HashMap::HashMapIterator::HashMapIterator(const HashMapIterator &it)
: _pMap(it._pMap),_iItem(it._iItem)
{
}

YAC_Multi_HashMap::HashMapIterator& YAC_Multi_HashMap::HashMapIterator::operator=(const HashMapIterator &it)
{
    if(this != &it)
    {
        _pMap       = it._pMap;
...
bool YAC_Multi_HashMap::HashMapIterator::operator==(const HashMapIterator& mcmi)
{
...
YAC_Multi_HashMap::HashMapIterator& YAC_Multi_HashMap::HashMapIterator::operator++()
{
	_iItem.nextItem();
	return (*this);
}

YAC_Multi_HashMap::HashMapIterator YAC_Multi_HashMap::HashMapIterator::operator++(int)
{
...
void YAC_Multi_HashMap::init(void *pAddr)
{
	_pHead          = static_cast<tagMapHead*>(pAddr);
	_pstModifyHead  = static_cast<tagModifyHead*>((void*)((char*)pAddr + sizeof(tagMapHead)));
}

void YAC_Multi_HashMap::initDataBlockSize(size_t iMinDataSize, size_t iMaxDataSize, float fFactor)
{
...
void YAC_Multi_HashMap::create(void *pAddr, size_t iSize)
{
	if(sizeof(tagHashItem)
	   + sizeof(tagMainKeyHashItem)
	   + sizeof(tagMapHead)
	   + sizeof(tagModifyHead)
	   + sizeof(YAC_MemMultiChunkAllocator::tagChunkAllocatorHead)
	   + 10 > iSize)
	{
        throw YAC_Multi_HashMap_Exception("[YAC_Multi_HashMap::create] mem size not enougth.");
    }

    if(_iMinDataSize == 0 || _iMaxDataSize == 0 || _fFactor < 1.0)
    {
        throw YAC_Multi_HashMap_Exception("[YAC_Multi_HashMap::create] init data size error:" + YAC_Common::tostr(_iMinDataSize) + "|" + YAC_Common::tostr(_iMaxDataSize) + "|" + YAC_Common::tostr(_fFactor));
    }
	...
	// 计算近似主key Hash个数
	size_t iMHashCount   = (iSize - sizeof(tagMapHead) - sizeof(tagModifyHead) - sizeof(YAC_MemChunkAllocator::tagChunkAllocatorHead)) / 
	...
	// 主key hash索引区
    size_t iHashMemSize = YAC_MemVector<tagMainKeyHashItem>::calcMemSize(iMHashCount);
	...
	// 联合hash索引区
	iHashMemSize = YAC_MemVector<tagHashItem>::calcMemSize(iUHashCount);
	if((char*)pHashAddr - (char*)_pHead + iHashMemSize > iSize)
	{
        throw YAC_Multi_HashMap_Exception("[YAC_Multi_HashMap::create] mem size not enougth.");
	}
...
void YAC_Multi_HashMap::connect(void *pAddr, size_t iSize)
{
	init(pAddr);
	...
	if(_pHead->_cMaxVersion != MAX_VERSION || _pHead->_cMinVersion != MIN_VERSION)
	{
        ...
        throw YAC_Multi_HashMap_Exception("[YAC_Multi_HashMap::connect] hash map version not equal:" + os.str() + " (data != code)");
	}

    if(_pHead->_iMemSize != iSize)
    {
		// 内存大小不匹配
        throw YAC_Multi_HashMap_Exception("[YAC_Multi_HashMap::connect] hash map size not equal:" + YAC_Common::tostr(_pHead->_iMemSize) + "!=" + YAC_Common::tostr(iSize));
    }
...
int YAC_Multi_HashMap::append(void *pAddr, size_t iSize)
{
    ...
    if(_pHead->_cMaxVersion != MAX_VERSION || _pHead->_cMinVersion != MIN_VERSION)
    {
        ...
        ostringstream os;
        os << (int)_pHead->_cMaxVersion << "." << (int)_pHead->_cMinVersion << " != " << ((int)MAX_VERSION) << "." << ((int)MIN_VERSION);
        throw YAC_Multi_HashMap_Exception("[YAC_Multi_HashMap::append] hash map version not equal:" + os.str() + " (data != code)");
    }
...
int YAC_Multi_HashMap::dump2file(const string &sFile)
{
...
int YAC_Multi_HashMap::load5file(const string &sFile)
{
...
int YAC_Multi_HashMap::checkMainKey(const string &mk)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
int YAC_Multi_HashMap::setFullData(const string &mk, bool bFull)
{
...
int YAC_Multi_HashMap::checkDirty(const string &mk, const string &uk)
{
	YAC_Multi_HashMap::FailureRecover recover(this);

	incGetCount();

	int ret          = YAC_Multi_HashMap::RT_OK;
	...
	if(it == end())
	{
		return YAC_Multi_HashMap::RT_NO_DATA;
	}
	...
	if (block.isDirty())
	{
		return YAC_Multi_HashMap::RT_DIRTY_DATA;
	}

	return YAC_Multi_HashMap::RT_OK;
}

int YAC_Multi_HashMap::checkDirty(const string &mk)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
int YAC_Multi_HashMap::setDirty(const string &mk, const string &uk)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
int YAC_Multi_HashMap::setClean(const string &mk, const string &uk)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
int YAC_Multi_HashMap::setSyncTime(const string &mk, const string &uk, time_t iSyncTime)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
void YAC_Multi_HashMap::incMainKeyBlockCount(const string &mk, bool bInc)
{
	uint32_t index = mhashIndex(mk);
...
void YAC_Multi_HashMap::updateMaxMainKeyBlockCount(size_t iCount)
{
...
int YAC_Multi_HashMap::get(const string &mk, const string &uk, Value &v)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
int YAC_Multi_HashMap::get(const string &mk, vector<Value> &vs)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
int YAC_Multi_HashMap::get(uint32_t &mh, map<string, vector<Value> > &vs)
{
	int ret = YAC_Multi_HashMap::RT_OK;
...
int YAC_Multi_HashMap::set(const string &mk, const string &uk, const string &v, 
...
int YAC_Multi_HashMap::set(const string &mk, const string &uk, DATATYPE eType, bool bHead, vector<Value> &vtData)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
int YAC_Multi_HashMap::set(const string &mk, vector<Value> &vtData)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
int YAC_Multi_HashMap::set(const vector<Value> &vtSet, DATATYPE eType, bool bHead, bool bForce, vector<Value> &vtErased)
{
	// 这里不需要加恢复代码
	//YAC_Multi_HashMap::FailureRecover recover(this);
...
int YAC_Multi_HashMap::del(const string &mk, const string &uk, Value &data)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
int YAC_Multi_HashMap::del(const string &mk, vector<Value> &data)
{
    if(_pHead->_bReadOnly) return RT_READONLY;
...
int YAC_Multi_HashMap::erase(int ratio, vector<Value> &vtData, bool bCheckDirty)
{
    if(_pHead->_bReadOnly) return RT_READONLY;
...
int YAC_Multi_HashMap::sync(time_t iNowTime, Value &data)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
void YAC_Multi_HashMap::backup(bool bForceFromBegin)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
int YAC_Multi_HashMap::backup(vector<Value> &vtData)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
YAC_Multi_HashMap::lock_iterator YAC_Multi_HashMap::begin()
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
YAC_Multi_HashMap::lock_iterator YAC_Multi_HashMap::rbegin()
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
YAC_Multi_HashMap::lock_iterator YAC_Multi_HashMap::beginSetTime()
{
	YAC_Multi_HashMap::FailureRecover recover(this);
	return lock_iterator(this, _pHead->_iSetHead, lock_iterator::IT_SET, lock_iterator::IT_NEXT);
}

YAC_Multi_HashMap::lock_iterator YAC_Multi_HashMap::rbeginSetTime()
{
	YAC_Multi_HashMap::FailureRecover recover(this);
	return lock_iterator(this, _pHead->_iSetTail, lock_iterator::IT_SET, lock_iterator::IT_PREV);
}

YAC_Multi_HashMap::lock_iterator YAC_Multi_HashMap::beginGetTime()
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
YAC_Multi_HashMap::lock_iterator YAC_Multi_HashMap::rbeginGetTime()
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
YAC_Multi_HashMap::lock_iterator YAC_Multi_HashMap::beginDirty()
{
	YAC_Multi_HashMap::FailureRecover recover(this);
	return lock_iterator(this, _pHead->_iDirtyTail, lock_iterator::IT_SET, lock_iterator::IT_PREV);
}

YAC_Multi_HashMap::hash_iterator YAC_Multi_HashMap::hashBegin()
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
string YAC_Multi_HashMap::desc()
{
	ostringstream s;
	{
        ...
        s << "[SingleBlockCount = "   << YAC_Common::tostr(singleBlockChunkCount())         << "]" << endl;
    ...
    vector<YAC_MemChunk::tagChunkHead> vChunkHead = _pDataAllocator->getBlockDetail();
...
size_t YAC_Multi_HashMap::eraseExcept(uint32_t iNowAddr, vector<Value> &vtData)
{
    //不能被淘汰
	...
	while (d != 0)
	{
        ...
        //判断按照哪种方式淘汰
        if(_pHead->_cEraseMode == YAC_Multi_HashMap::ERASEBYSET)
        {
			// 按set链淘汰
...
uint32_t YAC_Multi_HashMap::hashIndex(const string &mk, const string &uk)
{
	// mk是主key，uk是除主key外的辅key，二者加起来作为联合主键
	return hashIndex(mk + uk);
}

uint32_t YAC_Multi_HashMap::hashIndex(const string& k)
{
	return _hashf(k) % _hash.size();
}

uint32_t YAC_Multi_HashMap::mhashIndex(const string &mk)
{
	if(_mhashf)
	{
...
YAC_Multi_HashMap::lock_iterator YAC_Multi_HashMap::find(const string &mk, const string &uk)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
YAC_Multi_HashMap::lock_iterator YAC_Multi_HashMap::find(const string &mk, const string &uk, uint32_t index, Value &v, int &ret)
{
	ret = YAC_Multi_HashMap::RT_OK;
...
YAC_Multi_HashMap::lock_iterator YAC_Multi_HashMap::find(const string &mk, const string &uk, uint32_t index, int &ret)
{
	ret = YAC_Multi_HashMap::RT_OK;
...
uint32_t YAC_Multi_HashMap::find(const string &mk, uint32_t index, int &ret)
{
	ret = YAC_Multi_HashMap::RT_OK;
...
size_t YAC_Multi_HashMap::count(const string &mk)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
YAC_Multi_HashMap::lock_iterator YAC_Multi_HashMap::find(const string &mk)
{
	YAC_Multi_HashMap::FailureRecover recover(this);
...
void YAC_Multi_HashMap::analyseHash(uint32_t &iMaxHash, uint32_t &iMinHash, float &fAvgHash)
{
	iMaxHash = 0;
	iMinHash = (uint32_t)-1;
...
void YAC_Multi_HashMap::analyseHashM(uint32_t &iMaxHash, uint32_t &iMinHash, float &fAvgHash)
{
	iMaxHash = 0;
	iMinHash = (uint32_t)-1;
...
void YAC_Multi_HashMap::doRecover()
{
...
void YAC_Multi_HashMap::doUpdate()
{
    //==2,已经修改成功, 清除
    if(_pstModifyHead->_cModifyStatus == 1)
    {
...
size_t YAC_Multi_HashMap::getMinPrimeNumber(size_t n)
{
    while(true)
    {
        if(YAC_Common::isPrimeNumber(n))
        {
            return n;
        }
        ++n;
...
size_t YAC_Multi_HashMap::checkBadBlock(uint32_t iHash, bool bRepair)
{
	size_t iCount = 0;
	if(iHash >= _hash.size())
	{
