...
#include <errno.h>
#include <string.h>
#include "util/yac_encoder.h"

namespace util
{

void YAC_Encoder::gbk2utf8(char *sOut, int &iMaxOutLen, const char *sIn, int iInLen)
{
	char * pIn = (char*)sIn;
	...
	{
		throw YAC_Encoder_Exception("[YAC_Encoder::gbk2utf8] iInLen > iMaxOutLen error : ", errno);
	}

	cd = iconv_open("UTF-8","GBK");
	if (cd == (iconv_t)-1)
    {
        throw YAC_Encoder_Exception("[YAC_Encoder::gbk2utf8] iconv_open error : ", errno);
	}
...
string YAC_Encoder::gbk2utf8(const string &sIn)
{
	iconv_t cd;
...
void YAC_Encoder::gbk2utf8(const string &sIn, vector<string> &vtStr)
{
	iconv_t cd;
...
void YAC_Encoder::utf82gbk(char *sOut, int &iMaxOutLen, const char *sIn, int iInLen)
{
	iconv_t cd;
	...
	if (cd == (iconv_t)-1)
    {
        throw YAC_Encoder_Exception("[YAC_Encoder::utf82gbk] iconv_open error", errno);
	}
    ...
    if (ret == (size_t) - 1)
    {
        iMaxOutLen = 0;
        iconv_close(cd);
        throw YAC_Encoder_Exception("[YAC_Encoder::utf82gbk] iconv error", errno);
        return;
    }
 ...
 * \r -> \r\r
 */
string YAC_Encoder::transTo(const string& str, char f /*='\n'*/, char t /*= '\r'*/, char u /*= '\0'*/)
{
	string ret = str;
 ...
 * \r\r -> \r
 */
string YAC_Encoder::transFrom(const string& str, char f /*= '\n'*/, char t /*= '\r'*/, char u /*= '\0'*/)
{
	string ret = "";
