...
YAC_ThreadLock YAC_TimeProvider::g_tl;
YAC_TimeProviderPtr YAC_TimeProvider::g_tp = NULL;

YAC_TimeProvider* YAC_TimeProvider::getInstance()
{
    if(!g_tp)
    {
        YAC_ThreadLock::Lock lock(g_tl);

        if(!g_tp)
        {
            g_tp = new YAC_TimeProvider();
...
YAC_TimeProvider::~YAC_TimeProvider() 
{ 
    {
        YAC_ThreadLock::Lock lock(g_tl);
        _terminate = true; 
...
void YAC_TimeProvider::getNow(timeval *tv)  
{ 
    int idx = _buf_idx;
    *tv = _t[idx];
...
int64_t YAC_TimeProvider::getNowMs()
{
    struct timeval tv;
    getNow(&tv);
...
void YAC_TimeProvider::setTsc(timeval& tt)
{
    uint32_t low    = 0;
    uint32_t high   = 0;
    rdtsc(low,high);
...
void YAC_TimeProvider::addTimeOffset(timeval& tt,const int &idx)
{
    uint32_t low    = 0;
    uint32_t high   = 0;
    rdtsc(low,high);
    ...
    {
        //cerr<< "YAC_TimeProvider add_time_offset error,correct it by use gettimeofday. current_tsc|"<<current_tsc<<"|last_tsc|"<<_tsc[idx]<<endl;
        _use_tsc = false;
        ::gettimeofday(&tt, NULL);
        return;
    }
