...
string YAC_Common::trim(const string &sStr, const string &s, bool bChar)
{
    if(sStr.empty())
    {
        return sStr;
...
string YAC_Common::trimleft(const string &sStr, const string &s, bool bChar)
{
    if(sStr.empty())
    {
        return sStr;
...
string YAC_Common::trimright(const string &sStr, const string &s, bool bChar)
{
    if(sStr.empty())
    {
        return sStr;
...
string YAC_Common::lower(const string &s)
{
    string sString = s;
...
string YAC_Common::upper(const string &s)
{
    string sString = s;
...
bool YAC_Common::isdigit(const string &sInput)
{
...
int YAC_Common::str2tm(const string &sString, const string &sFormat, struct tm &stTm)
{
    char *p = strptime(sString.c_str(), sFormat.c_str(), &stTm);
    return (p != NULL) ? 0 : -1;
}

int YAC_Common::strgmt2tm(const string &sString, struct tm &stTm)
{
    return str2tm(sString, "%a, %d %b %Y %H:%M:%S GMT", stTm);
}

string YAC_Common::tm2str(const struct tm &stTm, const string &sFormat)
{
...
string YAC_Common::tm2str(const time_t &t, const string &sFormat)
{
    struct tm tt;
    localtime_r(&t, &tt);
...
string YAC_Common::now2str(const string &sFormat)
{
    time_t t = time(NULL);
    return tm2str(t, sFormat.c_str());
}

string YAC_Common::now2GMTstr()
{
    time_t t = time(NULL);
    return tm2GMTstr(t);
}

string YAC_Common::tm2GMTstr(const time_t &t)
{
    struct tm tt;
    gmtime_r(&t, &tt);
...
string YAC_Common::tm2GMTstr(const struct tm &stTm)
{
    return tm2str(stTm, "%a, %d %b %Y %H:%M:%S GMT");
}

string YAC_Common::nowdate2str()
{
    return now2str("%Y%m%d");
}

string YAC_Common::nowtime2str()
{
    return now2str("%H%M%S");
}

int64_t YAC_Common::now2ms()
{
    struct timeval tv;
...
string YAC_Common::bin2str(const void *buf, size_t len, const string &sSep, size_t lines)
{
    if(buf == NULL || len <=0 )
    {
        return "";
...
string YAC_Common::bin2str(const string &sBinData, const string &sSep, size_t lines)
{
    return bin2str((const void *)sBinData.data(), sBinData.length(), sSep, lines);
}

int YAC_Common::str2bin(const char *psAsciiData, unsigned char *sBinData, int iBinSize)
{
...
string YAC_Common::str2bin(const string &sString, const string &sSep, size_t lines)
{
...
char YAC_Common::x2c(const string &sWhat)
{
    register char digit;
...
string YAC_Common::replace(const string &sString, const string &sSrc, const string &sDest)
{
    if(sSrc.empty())
    {
        return sString;
...
bool YAC_Common::matchPeriod(const string& s, const string& pat)
{
    if(s.empty())
    {
        return false;
...
bool YAC_Common::matchPeriod(const string& s, const vector<string>& pat)
{
    for(size_t i = 0; i < pat.size(); i++)
    {
        if(YAC_Common::matchPeriod(s,pat[i]))
        {
            return true;
        }
    }
...
bool YAC_Common::isPrimeNumber(size_t n)
{
...
size_t YAC_Common::toSize(const string &s, size_t iDefaultSize)
{
    if (s.empty())
    {
        return iDefaultSize;
    ...
    char c = s[s.length()-1];
    if(c != 'K' && c != 'M' && c != 'G' && YAC_Common::trim(s) == YAC_Common::tostr(YAC_Common::strto<size_t>(s)))
    {
        //没有后缀, 且转换是正确的
        return (size_t)(YAC_Common::strto<size_t>(s));
    }
    else if(c == 'K' || c == 'M' || c == 'G')
    {
        if (s.length() == 1)
        ...
        float n = YAC_Common::strto<float>(s.substr(0, s.length()-1));

        if(YAC_Common::trim(s.substr(0, s.length()-1)) != YAC_Common::tostr(n))
        {
            return iDefaultSize;
        }
