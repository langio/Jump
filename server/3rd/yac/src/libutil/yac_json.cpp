...
JsonValueObjPtr YAC_Json::getObj(BufferJsonReader & reader)
{
...
JsonValueArrayPtr YAC_Json::getArray(BufferJsonReader & reader)
{
...
JsonValueNumPtr YAC_Json::getNum(BufferJsonReader & reader,char head)
{
    bool bOk=true;
    bool bFloat=false;
...
//为了提高效率和代码好写就先这么写了
JsonValueBooleanPtr YAC_Json::getBoolean(BufferJsonReader & reader,char c)
{
    bool bOk=false;
    bool bValue;
    if(c=='t'||c=='T')
...
JsonValuePtr YAC_Json::getNull(BufferJsonReader & reader,char c)
{
    assert(c=='n' || c=='N');
    bool bOk=false;
    c=reader.read();
...
uint16_t YAC_Json::getHex(BufferJsonReader & reader)
{
    uint16_t iCode=0;
    char c;
...
string YAC_Json::writeValue(const JsonValuePtr & p)
{
    if(!p)
    {
        return "null";
...
string YAC_Json::writeString(const JsonValueStringPtr & p)
{
    return writeString(p->value);
}

string YAC_Json::writeString(const string & s)
{
    string sReturn;
    string sReplace;
    sReturn="\"";
...
string YAC_Json::writeNum(const JsonValueNumPtr & p)
{
...
string YAC_Json::writeObj(const JsonValueObjPtr & p)
{
    string s;
    s="{ ";
...
string YAC_Json::writeArray(const JsonValueArrayPtr & p)
{
    string s;
    s="[ ";
...
string YAC_Json::writeBoolean(const JsonValueBooleanPtr & p)
{
    if(p->value)
        return "true";
    else
...
JsonValuePtr YAC_Json::getValue(const string & str)
{
