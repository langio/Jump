#include "util/yac_hashmap_compact.h"
...
int YAC_HashMapCompact::Block::getBlockData(YAC_HashMapCompact::BlockData &data)
{
	data._dirty = isDirty();
    ...
    if(ret != YAC_HashMapCompact::RT_OK)
    {
        return ret;
    }
    ...
    try
    {
        ...
        {
            return YAC_HashMapCompact::RT_ONLY_KEY;
        }
    }
    catch(exception &ex)
    {
        ret = YAC_HashMapCompact::RT_DECODE_ERR;
    }

    return ret;
}

uint32_t YAC_HashMapCompact::Block::getLastBlockHead()
{
...
int YAC_HashMapCompact::Block::get(void *pData, uint32_t &iDataLen)
{
	...
	if(!getBlockHead()->_bNextChunk)
	{
		...
		return YAC_HashMapCompact::RT_OK;
	}
	else
	{
		...
		{
			return YAC_HashMapCompact::RT_NOTALL_ERR;   //copy数据不完全
		}
		...
		while(iHasLen < iDataLen)
		{
			...
			if(!pChunk->_bNextChunk)
			{
				...
				{
					return YAC_HashMapCompact::RT_NOTALL_ERR;       //copy不完全
				}
...
int YAC_HashMapCompact::Block::get(string &s)
{
...
int YAC_HashMapCompact::Block::set(const string& k, const string& v, uint32_t iExpireTime, uint8_t iVersion, bool bNewBlock, bool bOnlyKey, vector<YAC_HashMapCompact::BlockData> &vtData)
{
	if(iVersion != 0 && getBlockHead()->_iVersion != iVersion)
	{
		// 数据版本不匹配
		return YAC_HashMapCompact::RT_DATA_VER_MISMATCH;
	}
...
void YAC_HashMapCompact::Block::setDirty(bool b)
{
...
bool YAC_HashMapCompact::Block::nextBlock()
{
...
bool YAC_HashMapCompact::Block::prevBlock()
{
...
void YAC_HashMapCompact::Block::deallocate()
{
...
void YAC_HashMapCompact::Block::insertHashMap()
{
...
void YAC_HashMapCompact::Block::makeNew(uint32_t index, uint32_t iAllocSize)
{
...
void YAC_HashMapCompact::Block::erase()
{
	//////////////////修改脏数据链表/////////////
	...
	{
		//如果是hash桶的头部, 则还需要处理
		YAC_HashMapCompact::tagHashItem *pItem  = _pMap->item(getBlockHead()->_iIndex);
...
void YAC_HashMapCompact::Block::refreshSetList()
{
...
void YAC_HashMapCompact::Block::refreshGetList()
{
...
void YAC_HashMapCompact::Block::deallocate(uint32_t iChunk)
{
...
int YAC_HashMapCompact::Block::allocate(uint32_t iDataLen, vector<YAC_HashMapCompact::BlockData> &vtData)
{
	uint32_t fn   = 0;
...
int YAC_HashMapCompact::Block::joinChunk(tagChunkHead *pChunk, const vector<uint32_t> chunks)
{
	// modified by smitchzhao @ 2011-05-25
...
int YAC_HashMapCompact::Block::allocateChunk(uint32_t fn, vector<uint32_t> &chunks, vector<YAC_HashMapCompact::BlockData> &vtData)
{
    assert(fn > 0);
	...
	while(true)
	{
		...
		if (t == 0)
		{
			...
			chunks.clear();
			return YAC_HashMapCompact::RT_NO_MEMORY;
		}
...
uint32_t YAC_HashMapCompact::Block::getDataLen()
{
	uint32_t n = 0;
	if(!getBlockHead()->_bNextChunk)
	{
...
uint32_t YAC_HashMapCompact::BlockAllocator::allocateMemBlock(uint32_t index, uint32_t &iAllocSize, vector<YAC_HashMapCompact::BlockData> &vtData)
{
begin:
    size_t iAddr;
...
uint32_t YAC_HashMapCompact::BlockAllocator::allocateChunk(uint32_t iAddr, uint32_t &iAllocSize, vector<YAC_HashMapCompact::BlockData> &vtData)
{
begin:
    size_t iIndex;
...
void YAC_HashMapCompact::BlockAllocator::deallocateMemBlock(const vector<uint32_t> &v)
{
...
void YAC_HashMapCompact::BlockAllocator::deallocateMemBlock(uint32_t v)
{
...
YAC_HashMapCompact::HashMapLockItem::HashMapLockItem(YAC_HashMapCompact *pMap, uint32_t iAddr)
: _pMap(pMap)
, _iAddr(iAddr)
{
}

YAC_HashMapCompact::HashMapLockItem::HashMapLockItem(const HashMapLockItem &mcmdi)
: _pMap(mcmdi._pMap)
, _iAddr(mcmdi._iAddr)
{
}

YAC_HashMapCompact::HashMapLockItem &YAC_HashMapCompact::HashMapLockItem::operator=(const YAC_HashMapCompact::HashMapLockItem &mcmdi)
{
    if(this != &mcmdi)
    {
        _pMap   = mcmdi._pMap;
...
bool YAC_HashMapCompact::HashMapLockItem::operator==(const YAC_HashMapCompact::HashMapLockItem &mcmdi)
{
...
bool YAC_HashMapCompact::HashMapLockItem::isDirty()
{
    Block block(_pMap, _iAddr);
    return block.isDirty();
}

bool YAC_HashMapCompact::HashMapLockItem::isOnlyKey()
{
    Block block(_pMap, _iAddr);
    return block.isOnlyKey();
}

uint32_t YAC_HashMapCompact::HashMapLockItem::getSyncTime()
{
    Block block(_pMap, _iAddr);
    return block.getSyncTime();
}

int YAC_HashMapCompact::HashMapLockItem::get(string& k, string& v)
{
	string s;
	...
	{
		return YAC_HashMapCompact::RT_EXCEPTION_ERR;
	}

	return YAC_HashMapCompact::RT_OK;
}

int YAC_HashMapCompact::HashMapLockItem::get(string& k)
{
	string s;
...
int YAC_HashMapCompact::HashMapLockItem::set(const string& k, const string& v, uint32_t iExpireTime, uint8_t iVersion, bool bNewBlock, vector<YAC_HashMapCompact::BlockData> &vtData)
{
	Block block(_pMap, _iAddr);
...
int YAC_HashMapCompact::HashMapLockItem::set(const string& k, uint8_t iVersion, bool bNewBlock, vector<YAC_HashMapCompact::BlockData> &vtData)
{
	Block block(_pMap, _iAddr);
...
bool YAC_HashMapCompact::HashMapLockItem::equal(const string &k, string &v, int &ret)
{
	string k1;
	ret = get(k1, v);

	if ((ret == YAC_HashMapCompact::RT_OK || ret == YAC_HashMapCompact::RT_ONLY_KEY) && k == k1)
	{
		return true;
	}
...
bool YAC_HashMapCompact::HashMapLockItem::equal(const string& k, int &ret)
{
	string k1;
	ret = get(k1);

	if (ret == YAC_HashMapCompact::RT_OK && k == k1)
	{
		return true;
	}
...
void YAC_HashMapCompact::HashMapLockItem::nextItem(int iType)
{
	Block block(_pMap, _iAddr);
...
void YAC_HashMapCompact::HashMapLockItem::prevItem(int iType)
{
	Block block(_pMap, _iAddr);
...
YAC_HashMapCompact::HashMapLockIterator::HashMapLockIterator()
: _pMap(NULL), _iItem(NULL, 0)
{
}

YAC_HashMapCompact::HashMapLockIterator::HashMapLockIterator(YAC_HashMapCompact *pMap, uint32_t iAddr, int iType, int iOrder)
: _pMap(pMap), _iItem(_pMap, iAddr), _iType(iType), _iOrder(iOrder)
{
}

YAC_HashMapCompact::HashMapLockIterator::HashMapLockIterator(const HashMapLockIterator &it)
: _pMap(it._pMap),_iItem(it._iItem), _iType(it._iType), _iOrder(it._iOrder)
{
}

YAC_HashMapCompact::HashMapLockIterator& YAC_HashMapCompact::HashMapLockIterator::operator=(const HashMapLockIterator &it)
{
    if(this != &it)
    {
        _pMap       = it._pMap;
...
bool YAC_HashMapCompact::HashMapLockIterator::operator==(const HashMapLockIterator& mcmi)
{
...
YAC_HashMapCompact::HashMapLockIterator& YAC_HashMapCompact::HashMapLockIterator::operator++()
{
	if(_iOrder == IT_NEXT)
	{
...
YAC_HashMapCompact::HashMapLockIterator YAC_HashMapCompact::HashMapLockIterator::operator++(int)
{
...
YAC_HashMapCompact::HashMapItem::HashMapItem(YAC_HashMapCompact *pMap, uint32_t iIndex)
: _pMap(pMap)
, _iIndex(iIndex)
{
}

YAC_HashMapCompact::HashMapItem::HashMapItem(const HashMapItem &mcmdi)
: _pMap(mcmdi._pMap)
, _iIndex(mcmdi._iIndex)
{
}

YAC_HashMapCompact::HashMapItem &YAC_HashMapCompact::HashMapItem::operator=(const YAC_HashMapCompact::HashMapItem &mcmdi)
{
    if(this != &mcmdi)
    {
        _pMap   = mcmdi._pMap;
...
bool YAC_HashMapCompact::HashMapItem::operator==(const YAC_HashMapCompact::HashMapItem &mcmdi)
{
...
void YAC_HashMapCompact::HashMapItem::get(vector<YAC_HashMapCompact::BlockData> &vtData)
{
    ...
    while(iAddr != 0)
    {
        ...
        YAC_HashMapCompact::BlockData data;
...
void YAC_HashMapCompact::HashMapItem::getExpire(uint32_t t, vector<YAC_HashMapCompact::BlockData> &vtData)
{
	uint32_t iAddr = _pMap->item(_iIndex)->_iBlockAddr;
...
void YAC_HashMapCompact::HashMapItem::nextItem()
{
...
int YAC_HashMapCompact::HashMapItem::setDirty()
{
	...
	while(iAddr != 0)
	{
		...
		if(!block.isOnlyKey())
		{
			YAC_HashMapCompact::FailureRecover check(_pMap);
...
YAC_HashMapCompact::HashMapIterator::HashMapIterator()
: _pMap(NULL), _iItem(NULL, 0)
{
}

YAC_HashMapCompact::HashMapIterator::HashMapIterator(YAC_HashMapCompact *pMap, uint32_t iIndex)
: _pMap(pMap), _iItem(_pMap, iIndex)
{
}

YAC_HashMapCompact::HashMapIterator::HashMapIterator(const HashMapIterator &it)
: _pMap(it._pMap),_iItem(it._iItem)
{
}

YAC_HashMapCompact::HashMapIterator& YAC_HashMapCompact::HashMapIterator::operator=(const HashMapIterator &it)
{
    if(this != &it)
    {
        _pMap       = it._pMap;
...
bool YAC_HashMapCompact::HashMapIterator::operator==(const HashMapIterator& mcmi)
{
...
YAC_HashMapCompact::HashMapIterator& YAC_HashMapCompact::HashMapIterator::operator++()
{
	_iItem.nextItem();
	return (*this);
}

YAC_HashMapCompact::HashMapIterator YAC_HashMapCompact::HashMapIterator::operator++(int)
{
...
void YAC_HashMapCompact::init(void *pAddr)
{
	_pHead          = static_cast<tagMapHead*>(pAddr);
	_pstModifyHead  = static_cast<tagModifyHead*>((void*)((char*)pAddr + sizeof(tagMapHead)));
}

void YAC_HashMapCompact::initDataBlockSize(uint32_t iMinDataSize, uint32_t iMaxDataSize, float fFactor)
{
...
void YAC_HashMapCompact::create(void *pAddr, size_t iSize)
{
	if(sizeof(tagHashItem) * 1
	   ...
	   + 10 > iSize)
	{
        throw YAC_HashMapCompact_Exception("[YAC_HashMapCompact::create] mem size not enougth.");
    }

    if(_iMinDataSize == 0 || _iMaxDataSize == 0 || _fFactor < 1.0)
    {
        throw YAC_HashMapCompact_Exception("[YAC_HashMapCompact::create] init data size error:" + YAC_Common::tostr(_iMinDataSize) + "|" + YAC_Common::tostr(_iMaxDataSize) + "|" + YAC_Common::tostr(_fFactor));
    }
    ...
    {
        throw YAC_HashMapCompact_Exception("[YAC_HashMapCompact::create] init block size error, block size must be less then " + YAC_Common::tostr((uint16_t)(-1) - sizeof(Block::tagBlockHead)));
    }
	...
	//Hash个数
	uint32_t iHashCount   = (iSize - sizeof(YAC_MemChunkAllocator::tagChunkAllocatorHead)) / ((uint32_t)(iBlockSize*_fRadio) + sizeof(tagHashItem));
    //采用最近的素数作为hash值
    iHashCount          = getMinPrimeNumber(iHashCount);

	void *pHashAddr     = (char*)_pHead + sizeof(tagMapHead) + sizeof(tagModifyHead);

    uint32_t iHashMemSize = YAC_MemVector<tagHashItem>::calcMemSize(iHashCount);
...
void YAC_HashMapCompact::connect(void *pAddr, size_t iSize)
{
	init(pAddr);
	...
	if(!_pHead->_bInit)
	{
		throw YAC_HashMapCompact_Exception("[YAC_HashMapCompact::connect] hash map created unsuccessfully, so can not be connected");
	}
	...
	if(_pHead->_cMaxVersion != MAX_VERSION || _pHead->_cMinVersion != MIN_VERSION)
	{
        ...
        throw YAC_HashMapCompact_Exception("[YAC_HashMapCompact::connect] hash map version not equal:" + os.str() + " (data != code)");
	}

    if(_pHead->_iMemSize != iSize)
    {
        throw YAC_HashMapCompact_Exception("[YAC_HashMapCompact::connect] hash map size not equal:" + YAC_Common::tostr(_pHead->_iMemSize) + "!=" + YAC_Common::tostr(iSize));
    }
...
int YAC_HashMapCompact::append(void *pAddr, size_t iSize)
{
    ...
    if(_pHead->_cMaxVersion != MAX_VERSION || _pHead->_cMinVersion != MIN_VERSION)
    {
        ostringstream os;
        os << (int)_pHead->_cMaxVersion << "." << (int)_pHead->_cMinVersion << " != " << ((int)MAX_VERSION) << "." << ((int)MIN_VERSION);
        throw YAC_HashMapCompact_Exception("[YAC_HashMapCompact::append] hash map version not equal:" + os.str() + " (data != code)");
    }
...
int YAC_HashMapCompact::dump2file(const string &sFile)
{
...
int YAC_HashMapCompact::load5file(const string &sFile)
{
...
int YAC_HashMapCompact::recover(size_t i, bool bRepair)
{
    doUpdate();
...
check:
    ...
    while(block.getHead() != 0)
    {
        BlockData data;
        int ret = block.getBlockData(data);
        if(ret != YAC_HashMapCompact::RT_OK && ret != YAC_HashMapCompact::RT_ONLY_KEY)
        {
            //增加删除block数
            ++e;
...
int YAC_HashMapCompact::checkDirty(const string &k)
{
	FailureRecover check(this);
	incGetCount();

	int ret         = YAC_HashMapCompact::RT_OK;
	...
	if(it == end())
	{
		return YAC_HashMapCompact::RT_NO_DATA;
	}
	...
	if (block.isDirty())
	{
		return YAC_HashMapCompact::RT_DIRTY_DATA;
	}

	return YAC_HashMapCompact::RT_OK;
}

int YAC_HashMapCompact::setDirty(const string& k)
{
	FailureRecover check(this);
...
int YAC_HashMapCompact::setDirtyAfterSync(const string& k)
{
	FailureRecover check(this);
...
int YAC_HashMapCompact::setClean(const string& k)
{
	FailureRecover check(this);
...
int YAC_HashMapCompact::get(const string& k, string &v, uint32_t &iSyncTime, uint32_t& iExpireTime, uint8_t& iVersion)
{
	FailureRecover check(this);
	incGetCount();
...
int YAC_HashMapCompact::get(const string& k, string &v, uint32_t &iSyncTime)
{
...
int YAC_HashMapCompact::get(const string& k, string &v)
{
    uint32_t iSyncTime;
	uint32_t iExpireTime;
	uint8_t iVersion;
...
int YAC_HashMapCompact::set(const string& k, const string& v, bool bDirty, vector<BlockData> &vtData)
{
	return set(k, v, 0, 0, bDirty, vtData);
}

int YAC_HashMapCompact::set(const string& k, const string& v, uint32_t iExpireTime, uint8_t iVersion, bool bDirty, vector<BlockData> &vtData)
{
	FailureRecover check(this);
	incGetCount();
...
int YAC_HashMapCompact::set(const string& k, vector<BlockData> &vtData)
{
	return set(k, 0, vtData);
}

int YAC_HashMapCompact::set(const string& k, uint8_t iVersion, vector<BlockData>& vtData)
{
	FailureRecover check(this);
	incGetCount();
...
int YAC_HashMapCompact::del(const string& k, BlockData &data)
{
	FailureRecover check(this);
	incGetCount();
...
int YAC_HashMapCompact::erase(int radio, BlockData &data, bool bCheckDirty)
{
    FailureRecover check(this);
    ...
    if(ret == YAC_HashMapCompact::RT_OK)
    {
        return YAC_HashMapCompact::RT_ERASE_OK;
    }
    return ret;
}

void YAC_HashMapCompact::sync()
{
    FailureRecover check(this);
...
int YAC_HashMapCompact::sync(uint32_t iNowTime, BlockData &data)
{
    FailureRecover check(this);
...
void YAC_HashMapCompact::backup(bool bForceFromBegin)
{
    FailureRecover check(this);
...
int YAC_HashMapCompact::backup(BlockData &data)
{
    FailureRecover check(this);
    ...
    if(ret == YAC_HashMapCompact::RT_OK)
    {
        return YAC_HashMapCompact::RT_NEED_BACKUP;
    }
    return ret;
}

YAC_HashMapCompact::lock_iterator YAC_HashMapCompact::find(const string& k)
{
    FailureRecover check(this);
...
YAC_HashMapCompact::lock_iterator YAC_HashMapCompact::begin()
{
	FailureRecover check(this);
...
YAC_HashMapCompact::lock_iterator YAC_HashMapCompact::rbegin()
{
	FailureRecover check(this);
...
string YAC_HashMapCompact::desc()
{
	ostringstream s;
	{
...
uint32_t YAC_HashMapCompact::eraseExcept(uint32_t iNowAddr, vector<BlockData> &vtData)
{
    //在真正数据淘汰前使修改生效
    doUpdate();
	...
	while (d != 0)
	{
        ...
        //判断按照哪种方式淘汰
        if(_pHead->_cEraseMode == YAC_HashMapCompact::ERASEBYSET)
        {
        ...
        else if(ret == YAC_HashMapCompact::RT_NO_DATA)
        {
            d--;
        }
...
uint32_t YAC_HashMapCompact::hashIndex(const string& k)
{
	return _hashf(k) % _hash.size();
}

YAC_HashMapCompact::lock_iterator YAC_HashMapCompact::find(const string& k, uint32_t index, string &v, int &ret)
{
	ret = YAC_HashMapCompact::RT_OK;
...
YAC_HashMapCompact::lock_iterator YAC_HashMapCompact::find(const string& k, uint32_t index, int &ret)
{
	ret = YAC_HashMapCompact::RT_OK;
...
void YAC_HashMapCompact::analyseHash(uint32_t &iMaxHash, uint32_t &iMinHash, float &fAvgHash)
{
	iMaxHash = 0;
	iMinHash = (uint32_t)-1;
...
void YAC_HashMapCompact::saveAddr(uint32_t iAddr, char cByte)
{
...
void YAC_HashMapCompact::doRecover()
{
...
void YAC_HashMapCompact::doUpdate()
{
    if(_pstModifyHead->_cModifyStatus == 1)
    {
        _pstModifyHead->_iNowIndex        = 0;
...
void YAC_HashMapCompact::doUpdate2()
{
	if(_pstModifyHead->_cModifyStatus == 1)
	{
...
void YAC_HashMapCompact::doUpdate3()
{
...
void YAC_HashMapCompact::doUpdate4()
{
	assert(_pstModifyHead->_cModifyStatus == 1);
...
uint32_t YAC_HashMapCompact::getMinPrimeNumber(uint32_t n)
{
    while(true)
    {
...
void YAC_HashMapCompact::deallocate(uint32_t iChunk, uint32_t iIndex)
{
...
void YAC_HashMapCompact::deallocate2(uint32_t iHead)
{
