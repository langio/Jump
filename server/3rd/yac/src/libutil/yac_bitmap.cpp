...
const int YAC_BitMap::BitMap::_magic_bits[8]={0x80,0x40,0x20,0x10,0x8,0x4,0x2,0x1};

size_t YAC_BitMap::BitMap::calcMemSize(size_t iElementCount)
{
...
void YAC_BitMap::BitMap::create(void *pAddr, size_t iSize)
{
...
int YAC_BitMap::BitMap::connect(void *pAddr, size_t iSize)
{
    _pHead = static_cast<tagBitMapHead*>(pAddr);
...
int YAC_BitMap::BitMap::get(size_t i)
{
...
int YAC_BitMap::BitMap::set(size_t i)
{
    if(i/8 >= (_pHead->_iMemSize-sizeof(tagBitMapHead)))
    {
        return -1;
...
int YAC_BitMap::BitMap::clear(size_t i)
{
    if(i/8 >= (_pHead->_iMemSize-sizeof(tagBitMapHead)))
    {
        return -1;
...
int YAC_BitMap::BitMap::clear4all()
{
...
int YAC_BitMap::BitMap::dump2file(const string &sFile)
{
...
int YAC_BitMap::BitMap::load5file(const string &sFile)
{
...
size_t YAC_BitMap::calcMemSize(size_t iElementCount, unsigned iBitCount)
{
    size_t n = BitMap::calcMemSize(iElementCount);
    if(n * iBitCount < n)
    {
        throw YAC_BitMap_Exception("[YAC_BitMap::calcMemSize] memory to much error"); 
    }
    return n * iBitCount;
}

void YAC_BitMap::create(void *pAddr, size_t iSize, unsigned iBitCount)
{
    assert(iBitCount != 0);
...
int YAC_BitMap::connect(void *pAddr, size_t iSize, unsigned iBitCount)
{
    assert(iBitCount != 0);
    assert(iSize % iBitCount == 0);
...
int YAC_BitMap::get(size_t i, unsigned iBit)
{
    assert(iBit != 0);

    if(iBit > _bitmaps.size())
    {
        throw YAC_BitMap_Exception("[YAC_BitMap::get] bit beyond range:"+YAC_Common::tostr(iBit)+">"+YAC_Common::tostr(_bitmaps.size())); 
    }

    return _bitmaps[iBit-1].get(i);
}

int YAC_BitMap::set(size_t i, unsigned iBit)
{
    assert(iBit != 0);
...
int YAC_BitMap::clear(size_t i, unsigned iBit)
{
    assert(iBit != 0);
...
int YAC_BitMap::clear4all(unsigned iBit)
{
    assert(iBit != 0);
...
int YAC_BitMap::dump2file(const string &sFile)
{
    FILE *fp = fopen(sFile.c_str(), "wb");
    if(fp == NULL)
    {
...
int YAC_BitMap::load5file(const string &sFile)
{
    FILE *fp = fopen(sFile.c_str(), "rb");
    if(fp == NULL)
    {
