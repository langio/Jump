...
ifstream::pos_type YAC_File::getFileSize(const string &sFullFileName)
{
...
bool YAC_File::isAbsolute(const string &sFullFileName)
{
...
bool YAC_File::isFileExist(const string &sFullFileName, mode_t iFileType)
{
    struct stat f_stat;
...
bool YAC_File::isFileExistEx(const string &sFullFileName, mode_t iFileType)
{
    struct stat f_stat;
...
bool YAC_File::makeDir(const string &sDirectoryPath, mode_t iFlag)
{
...
bool YAC_File::makeDirRecursive(const string &sDirectoryPath, mode_t iFlag)
{
...
int YAC_File::setExecutable(const string &sFullFileName, bool canExecutable)
{
    struct stat f_stat;
...
bool YAC_File::canExecutable(const string &sFullFileName)
{
    struct stat f_stat;
...
int YAC_File::removeFile(const string &sFullFileName, bool bRecursive)
{
...
string YAC_File::simplifyDirectory(const string& path)
{
    string result = path;
...
string YAC_File::load2str(const string &sFullFileName)
{
    ifstream ifs(sFullFileName.c_str());
...
void YAC_File::save2file(const string &sFullFileName, const string &sFileData)
{
...
int YAC_File::save2file(const string &sFullFileName, const char *sFileData, size_t length)
{
...
string YAC_File::getExePath()
{
    ...
    if ( count < 0 )
    {
        throw YAC_File_Exception("[YAC_File::getExePath] could not get exe path error", errno);
    }
...
string YAC_File::extractFileName(const string &sFullFileName)
{
...
string YAC_File::extractFilePath(const string &sFullFileName)
{
    if(sFullFileName.length() <= 0)
    {
        return "./";
...
string YAC_File::extractFileExt(const string &sFullFileName)
{
    string::size_type pos;
...
string YAC_File::excludeFileExt(const string &sFullFileName)
{
    string::size_type pos;
    if ((pos = sFullFileName.rfind('.')) == string::npos)
    {
...
string YAC_File::replaceFileExt(const string &sFullFileName, const string &sExt)
{
    return excludeFileExt(sFullFileName) + "." + sExt;
}

string YAC_File::extractUrlFilePath(const string &sUrl)
{
    string sLowerUrl = YAC_Common::lower(sUrl);
...
size_t YAC_File::scanDir(const string &sFilePath, vector<string> &vtMatchFiles, FILE_SELECT f, int iMaxSize )
{
    vtMatchFiles.clear();
...
void YAC_File::listDirectory(const string &path, vector<string> &files, bool bRecursive)
{
    vector<string> tf;
    scanDir(path, tf, 0, 0);
...
void YAC_File::copyFile(const string &sExistFile, const string &sNewFile,bool bRemove)
{
    if(YAC_File::isFileExist(sExistFile, S_IFDIR))
    {    
        YAC_File::makeDir(sNewFile);  
        vector<string> tf;
        YAC_File::scanDir(sExistFile,tf, 0, 0);
        for(size_t i = 0; i <tf.size(); i++)
        {
            if(tf[i] == "." || tf[i] == "..")
            continue;
    ...
    {   
        ...
        if(!fin)
        {      
            throw YAC_File_Exception("[YAC_File::copyFile] error", errno);
        }
