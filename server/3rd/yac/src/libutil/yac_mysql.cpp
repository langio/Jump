...
YAC_Mysql::YAC_Mysql(const string& sHost, const string& sUser, const string& sPasswd, const string& sDatabase, const string &sCharSet, int port, int iFlag)
:_bConnected(false)
{
...
YAC_Mysql::YAC_Mysql(const YAC_DBConf& tcDBConf)
:_bConnected(false)
{
    _dbConf = tcDBConf;
...
void YAC_Mysql::init(const string& sHost, const string& sUser, const string& sPasswd, const string& sDatabase, const string &sCharSet, int port, int iFlag)
{
    _dbConf._host = sHost;
    _dbConf._user = sUser;
...
void YAC_Mysql::init(const YAC_DBConf& tcDBConf)
{
    _dbConf = tcDBConf;
}

void YAC_Mysql::connect()
{
    disconnect();
    ...
    if(!_dbConf._charset.empty())  {
        if (mysql_options(_pstMql, MYSQL_SET_CHARSET_NAME, _dbConf._charset.c_str())) {
    		throw YAC_Mysql_Exception(string("YAC_Mysql::connect: mysql_options MYSQL_SET_CHARSET_NAME ") + _dbConf._charset + ":" + string(mysql_error(_pstMql)));
        }
    }
    ...
    {
        throw YAC_Mysql_Exception("[YAC_Mysql::connect]: mysql_real_connect: " + string(mysql_error(_pstMql)));
    }

	_bConnected = true;
}

void YAC_Mysql::disconnect()
{
    if (_pstMql != NULL)
    {
        mysql_close(_pstMql);
...
string YAC_Mysql::escapeString(const string& sFrom)
{
	if(!_bConnected)
	{
		connect();
...
MYSQL *YAC_Mysql::getMysql(void)
{
    return _pstMql;
}

string YAC_Mysql::buildInsertSQL(const string &sTableName, const RECORD_DATA &mpColumns)
{
...
string YAC_Mysql::buildReplaceSQL(const string &sTableName, const RECORD_DATA &mpColumns)
{
    ostringstream sColumnNames;
    ostringstream sColumnValues;
...
string YAC_Mysql::buildUpdateSQL(const string &sTableName,const RECORD_DATA &mpColumns, const string &sWhereFilter)
{
...
string YAC_Mysql::getVariables(const string &sName)
{
...
void YAC_Mysql::execute(const string& sSql)
{
	/**
	没有连上, 连接数据库
	*/
    ...
    if (iRet != 0)
    {
        throw YAC_Mysql_Exception("[YAC_Mysql::execute]: mysql_query: [ " + sSql+" ] :" + string(mysql_error(_pstMql)));
    }
}

YAC_Mysql::MysqlData YAC_Mysql::queryRecord(const string& sSql)
{
    MysqlData   data;
	...
	*/
    ...
    if(pstRes == NULL)
    {
        throw YAC_Mysql_Exception("[YAC_Mysql::queryRecord]: mysql_store_result: " + sSql + " : " + string(mysql_error(_pstMql)));
    }
...
size_t YAC_Mysql::updateRecord(const string &sTableName, const RECORD_DATA &mpColumns, const string &sCondition)
{
...
size_t YAC_Mysql::insertRecord(const string &sTableName, const RECORD_DATA &mpColumns)
{
...
size_t YAC_Mysql::replaceRecord(const string &sTableName, const RECORD_DATA &mpColumns)
{
...
size_t YAC_Mysql::deleteRecord(const string &sTableName, const string &sCondition)
{
    ostringstream sSql;
...
size_t YAC_Mysql::getRecordCount(const string& sTableName, const string &sCondition)
{
    ostringstream sSql;
...
size_t YAC_Mysql::getSqlCount(const string &sCondition)
{
    ostringstream sSql;
...
int YAC_Mysql::getMaxValue(const string& sTableName, const string& sFieldName,const string &sCondition)
{
    ostringstream sSql;
...
bool YAC_Mysql::existRecord(const string& sql)
{
    return queryRecord(sql).size() > 0;
}

long YAC_Mysql::lastInsertID()
{
    return mysql_insert_id(_pstMql);
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////
YAC_Mysql::MysqlRecord::MysqlRecord(const map<string, string> &record)
: _record(record)
{
}

const string& YAC_Mysql::MysqlRecord::operator[](const string &s)
{
    map<string, string>::const_iterator it = _record.find(s);
    if(it == _record.end())
    {
        throw YAC_Mysql_Exception("field '" + s + "' not exists.");
    }
    return it->second;
}
...
vector<map<string, string> >& YAC_Mysql::MysqlData::data()
{
    return _data;
}

size_t YAC_Mysql::MysqlData::size()
{
    return _data.size();
}

YAC_Mysql::MysqlRecord YAC_Mysql::MysqlData::operator[](size_t i)
{
