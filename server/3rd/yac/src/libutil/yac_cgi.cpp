...
ostream &operator<<(ostream &os, const YAC_Cgi_Upload &tcCgiUpload)
{
    os << tcCgiUpload.tostr();
    return os;
}

YAC_Cgi_Upload::YAC_Cgi_Upload(const YAC_Cgi_Upload &tcCgiUpload)
{
...
YAC_Cgi_Upload & YAC_Cgi_Upload::operator=(const YAC_Cgi_Upload &tcCgiUpload)
{
    if(this != &tcCgiUpload)
    {
        _sFileName      = tcCgiUpload._sFileName;
...
string YAC_Cgi_Upload::tostr() const
{
    string sBuffer;
    ...
    sBuffer += " [大小(字节):" + YAC_Common::tostr(_iSize) + "] ";
    sBuffer += " [超过大小限制:" + YAC_Common::tostr(_bOverSize) + "] ";
...
void YAC_Cgi::setUpload(const string &sUploadFilePrefix, int iMaxUploadFiles, size_t iUploadMaxSize, size_t iMaxContentLength)
{
...
void YAC_Cgi::parseCgi(const YAC_HttpRequest &request)
{
    setCgiEnv("QUERY_STRING", request.getRequestParam());
    setCgiEnv("CONTENT_LENGTH", YAC_Common::tostr(request.getContentLength()));
...
void YAC_Cgi::getPOST(string &sBuffer)
{
    string::size_type iSize = atoi(getCgiEnv(YAC_Cgi::ENM_CONTENT_LENGTH).c_str());

    if( iSize > _iMaxContentLength)
    {
        throw YAC_Cgi_Exception("[YAC_Cgi::getPOST] : CONTENT_LENGTH is too large!");
    }
    ...
    {
        throw YAC_Cgi_Exception("[YAC_Cgi::getPOST] : Read CONTENT error: content size " + YAC_Common::tostr(sBuffer.length()) + "<" + YAC_Common::tostr(iSize));
    }
}

void YAC_Cgi::getGET(string &sBuffer)
{
    sBuffer = getCgiEnv(YAC_Cgi::ENM_QUERY_STRING);
}

void YAC_Cgi::readCgiInput(multimap<string, string> &mmpParams, map<string, string> &mpCooies)
{
    string sBuffer = YAC_Cgi::getCgiEnv(YAC_Cgi::ENM_HTTP_COOKIE);
...
void YAC_Cgi::parseNormal(multimap<string, string> &mmpParams, const string& sBuffer)
{
    int iFlag = 0;
    string sName;
    string sValue;
...
void YAC_Cgi::parseCookies(map<string, string> &mpCookies, const string& sBuffer)
{
    string::size_type len = sBuffer.length();
    string::size_type pos = 0;
    ...
    while(pos < len)
    {
        if(sBuffer[pos] == '=')
        {
            if( pos == len - 1)                     //最后一个参数
            {
                sName  = decodeURL(YAC_Common::trimleft(sName, " "));
...
void YAC_Cgi::ignoreLine()
{
    ...
    while(true)
    {
        ...
        {
            throw YAC_Cgi_Exception("[YAC_Cgi::parseFormData] 'multipart/form-data' Format is error");
        }
        sBuffer = YAC_Common::trim(sBuffer, "\r\n");
...
bool YAC_Cgi::writeFile(FILE*fp, const string &sFileName, const string &sBuffer, size_t &iTotalWrite)
{
    if(!fp)
    {
        return false;
    ...
    {
        fclose(fp);
        throw YAC_Cgi_Exception("[YAC_Cgi::parseFormData] upload file '" + _mpUpload[sFileName]._sServerFileName + "' error:" + string(strerror(errno)));
    }
...
void YAC_Cgi::parseFormData(multimap<string, string> &mmpParams, const string &sBoundary)
{
    string sBuffer;
    if(!getline(*_is, sBuffer))
    {
    ...
    if(pos1 != string::npos)
    {
        ...
        if( sValue.length() > 0)
        {
            if(_iMaxUploadFiles < 0 || _mpUpload.size() < (size_t)_iMaxUploadFiles)
            {
                ...
                //组成文件名称
                string sUploadFileName = _sUploadFilePrefix + "_" + YAC_Common::tostr(_mpUpload.size());
                ...
                if ( (fp = fopen(sUploadFileName.c_str(),"w")) == NULL)
                {
                    ...
                    throw YAC_Cgi_Exception("[YAC_Cgi::parseFormData] Upload File '" + sValue + "' to '" + sUploadFileName +"' error! " + string(strerror(errno)));
                }
            }
            else
            {
        ...
        {
            string sTmp = YAC_Common::trimright(sBuffer);
...
void YAC_Cgi::parseUpload(multimap<string, string> &mmpParams)
{
    string::size_type iSize = atoi(getCgiEnv(YAC_Cgi::ENM_CONTENT_LENGTH).c_str());

    if( iSize > _iMaxContentLength)
    {
        throw YAC_Cgi_Exception("[YAC_Cgi::parseUpload] : CONTENT_LENGTH is too large!");
    }

    //get content type
    string sContentType = getCgiEnv(YAC_Cgi::ENM_CONTENT_TYPE);
    ...
    {
        throw YAC_Cgi_Exception("[YAC_Cgi::parseUpload] 'multipart/form-data' Format is error");
    }
}

string &YAC_Cgi::operator[](const string &sName)
{
...
string YAC_Cgi::getValue(const string& sName) const
{
...
const vector<string>& YAC_Cgi::getMultiValue(const string& sName, vector<string> &vtValue) const
{
    vtValue.clear();
...
const multimap<string, string> &YAC_Cgi::getParamMap() const
{
    return _mmpParams;
}

map<string, string> YAC_Cgi::getParamMapEx() const
{
...
const map<string, string> &YAC_Cgi::getCookiesMap() const
{
    return _mpCookies;
}

bool  YAC_Cgi::isUploadOverSize() const
{
	return _bUploadFileOverSize;
}

bool  YAC_Cgi::isUploadOverSize(vector<YAC_Cgi_Upload> &vtUploads) const
{
    vtUploads.clear();

    map<string, YAC_Cgi_Upload>::const_iterator itEnd = _mpUpload.end();

    for( map<string, YAC_Cgi_Upload>::const_iterator it = _mpUpload.begin(); it != itEnd; ++it)
    {
...
const map<string, YAC_Cgi_Upload> &YAC_Cgi::getUploadFilesMap() const
{
    return _mpUpload;
}

size_t YAC_Cgi::getUploadFilesCount() const
{
    return _mpUpload.size();
}

string YAC_Cgi::getCookie(const string &sName) const
{
...
string YAC_Cgi::setCookie(const string &sName, const string &sValue, const string &sExpires, const string &sPath, const string &sDomain, bool bSecure)
{
    if(sName.length() == 0)
    {
        return "";
...
bool YAC_Cgi::isParamEmpty() const
{
    return _mmpParams.empty();
}

bool YAC_Cgi::isParamExist(const string& sName) const
{
    return _mmpParams.find(sName) != _mmpParams.end();
}

string YAC_Cgi::getCgiEnv(int iEnv)
{
    switch(iEnv)
    {
...
string YAC_Cgi::getCgiEnv(const string &sEnv)
{
...
void YAC_Cgi::setCgiEnv(const string &sName, const string &sValue)
{
    _env[sName] = sValue;
}

string YAC_Cgi::htmlHeader(const string &sHeader)
{
	return "Content-type: " + sHeader + "\n\n";
}

string YAC_Cgi::decodeURL(const string &sUrl)
{
    string sDecodeUrl;
    ...
    while (pos < len)
    {
        if(sUrl[pos] == '%')
        {
            sDecodeUrl += YAC_Common::x2c(sUrl.substr(pos + 1));
            pos += 3;
        }
        else
        {
...
string YAC_Cgi::encodeURL(const string &sUrl)
{
...
string YAC_Cgi::encodeHTML(const string &src, bool blankEncode)
{
    if (src == "")
        return "";

    string result = src;
    result = YAC_Common::replace(result, "&", "&amp;");
    result = YAC_Common::replace(result, "<", "&lt;");
    result = YAC_Common::replace(result, ">", "&gt;");
    result = YAC_Common::replace(result, "\"", "&quot;");
...
string YAC_Cgi::encodeXML(const string &src)
{
    if (src == "")
        return "";
